...
export function useDocAI() {
  ...
  // Convenience Methods
  const analyzeDoc = (docs: Record<string, string>) => callDocAI({ action: "analyze", docs });

  const validateDocs = (docs: Record<string, string>) => callDocAI({ action: "validate", docs });

  const syncDesign = (designReferences: Record<string, string>) =>
    callDocAI({ action: "sync-design", designReferences });

  const updateDocs = (docs: Record<string, string>) => callDocAI({ action: "update", docs });
