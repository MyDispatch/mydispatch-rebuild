...
interface UseChatMessagesReturn {
  ...
  sendFileMessage: (message: Omit<ChatMessage, 'message_type'> & { file_url: string }) => Promise<void>;
  isSending: boolean;
}
...
export function useChatMessages(conversationId: string): UseChatMessagesReturn {
  ...
  const sendMessage = async (message: Omit<ChatMessage, 'message_type' | 'file_url'>): Promise<void> => {
  ...
  const sendFileMessage = async (message: Omit<ChatMessage, 'message_type'> & { file_url: string }): Promise<void> => {
    await sendMessageMutation.mutateAsync({
      ...message,
      message_type: 'file',
    });
