    ...
    const validateCompliance = () => {
      ...
      if (options.hasForm) {
        ...
        forms.forEach((form, index) => {
          const hasPrivacyHint = form.querySelector('[data-privacy-hint]') ||
      ...
      if (options.hasAI) {
        ...
        if (aiElements.length === 0) {
          warnings.push(
            'AI Act: KI-generierte Inhalte fehlen Kennzeichnung (data-ai-generated)'
          );
        }
      }
      ...
      if (options.hasFooter !== false) {
        ...
        if (footer) {
          ...
          requiredLinks.forEach((linkText) => {
            const hasLink = Array.from(footer.querySelectorAll('a')).some(
      ...
      if (warnings.length > 0) {
        ...
        warnings.forEach((warning) => logger.warn(warning, { component: 'useLegalComplianceValidation' }));
        logger.info('Siehe: docs/RECHTLICHE_COMPLIANCE_VORGABEN_V18.5.1.md', { component: 'useLegalComplianceValidation' });
        logger.groupEnd();
      }
    };
