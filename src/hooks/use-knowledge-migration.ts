...
export function useKnowledgeMigration() {
  ...
  const updateProgress = (phase: string, current: number, total: number, status: MigrationProgress['status'], message: string) => {
    setProgress(prev => {
  ...
  const migrateDocumentation = async () => {
    ...
    try {
      ...
      updateProgress(phase, processed, criticalDocs.length, 'completed', `✅ ${inserted} entries added`);
      ...
      const knownSolutions = [
        {
          ...
          prevention_checklist: ['Check for nested interactive elements', 'Use useCallback for handlers']
        },
        {
  ...
  const runValidation = async () => {
    ...
    try {
      ...
      updateProgress(phase, 4, 4, 'completed', `✅ Validation complete: ${kbCount || 0} KB | ${compCount || 0} Components | ${issuesCount || 0} Issues | ${snippetsCount || 0} Snippets`);
