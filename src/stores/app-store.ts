  ...
  devtools(
    persist(
      (set, get) => ({
        ...
        // ========== USER ACTIONS ==========
        setUser: (user) =>
          set((state) => ({ user: { ...state.user, data: user } }), false, "setUser"),

        setUserLoading: (loading) =>
          set((state) => ({ user: { ...state.user, loading } }), false, "setUserLoading"),

        setUserError: (error) =>
          set((state) => ({ user: { ...state.user, error } }), false, "setUserError"),

        clearUser: () =>
          set({ user: { data: null, loading: false, error: null } }, false, "clearUser"),

        // ========== FILTER ACTIONS ==========
        setFilter: (key, value) =>
          set((state) => ({ filters: { ...state.filters, [key]: value } }), false, "setFilter"),
        ...
        addToast: (toast) =>
          set(
            (state) => ({
              ui: {
                ...state.ui,
                toasts: [...state.ui.toasts, { ...toast, id: Math.random().toString(36).slice(2) }],
              },
            }),
            false,
            "addToast"
        ...
        setRealtimeActive: (active) =>
          set((state) => ({ realtime: { ...state.realtime, active } }), false, "setRealtimeActive"),

        addRealtimeChannel: (name, channel) =>
          set(
            (state) => {
