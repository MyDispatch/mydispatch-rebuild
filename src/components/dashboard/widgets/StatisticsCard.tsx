    ...
    <Card className="bg-white border border-slate-200 shadow-sm hover:shadow-md transition-all duration-200">
      <CardHeader className="pb-3">
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-sm font-semibold text-slate-900">Vergleich & Trends</CardTitle>
        </div>
      </CardHeader>
      <CardContent className="pb-3 space-y-3">
        {periods.map((period, index) => {
          const TrendIcon =
            period.trend === "up" ? TrendingUp : period.trend === "down" ? TrendingDown : null;
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                {TrendIcon && (
                  ...
                  >
                    ...
                    <span className="text-[10px] font-bold">{Math.abs(period.change)}%</span>
                  </Badge>
                )}
              </div>
              ...
              <div className="flex items-baseline justify-between">
                <div>
                  ...
                  <p className="text-[10px] text-slate-600 mt-0.5">Aktuell</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-semibold text-slate-600">
                    {valueFormatter(period.previous)}
                  </p>
                  <p className="text-[10px] text-slate-500 mt-0.5">Vorher</p>
                </div>
              </div>
            </div>
          );
