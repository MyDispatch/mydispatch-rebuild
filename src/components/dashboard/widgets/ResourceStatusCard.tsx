...
export function ResourceStatusCard({ stats }: ResourceStatusCardProps) {
  const driverUtilization =
    stats.drivers.total > 0 ? Math.round((stats.drivers.busy / stats.drivers.total) * 100) : 0;

  const vehicleUtilization =
    stats.vehicles.total > 0 ? Math.round((stats.vehicles.inUse / stats.vehicles.total) * 100) : 0;
    ...
    <Card className="bg-white border border-slate-200 shadow-sm hover:shadow-md transition-all duration-200">
      <CardHeader className="pb-3">
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-sm font-semibold text-slate-900">Fahrer-Status</CardTitle>
        </div>
      </CardHeader>
      <CardContent className="pb-3 space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-bold text-slate-900">
              {driverUtilization}% ausgelastet
            </span>
          </div>
          <Progress value={driverUtilization} className="h-2" />
          <div className="grid grid-cols-3 gap-2 mt-2">
            <div className="p-2 rounded-lg bg-green-50 border border-green-200">
              <p className="text-[10px] font-semibold text-green-600">Verfügbar</p>{" "}
              {/* ✅ Status Exception */}
            ...
            <div className="p-2 rounded-lg bg-yellow-50 border border-yellow-200">
              <p className="text-[10px] font-semibold text-yellow-600">Im Einsatz</p>{" "}
              {/* ✅ Status Exception */}
        ...
        <div className="space-y-2 pt-3 border-t border-slate-200">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-bold text-slate-900">
              {vehicleUtilization}% ausgelastet
            </span>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 mt-2">
            ...
            <div className="p-2 rounded-lg bg-red-50 border border-red-200">
              <p className="text-[10px] font-semibold text-red-600">Wartung</p>{" "}
              {/* ✅ Status Exception */}
