...
export function useTrafficData() {
  ...
  if (hour >= 7 && hour <= 9) {
    ...
    return {
      status: "mittel",
      label: "Feierabendverkehr",
      color: CI_COLORS_HEX.statusWarning,
      icon: TrendingUp,
    };
  } else {
    return {
      status: "normal",
      label: "Fließend",
      color: CI_COLORS_HEX.statusSuccess,
      icon: TrendingDown,
    };
  }
}
...
export function TrafficDisplay() {
  ...
  return (
    <div className="flex items-center gap-2 shrink-0">
      ...
      <span className="text-xs font-medium text-slate-700">{traffic.label}</span>
    </div>
  );
}
