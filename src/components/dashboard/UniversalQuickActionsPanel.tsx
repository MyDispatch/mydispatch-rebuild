    ...
    >
      ...
      <Card className="border-slate-300 bg-white/95 shadow-lg hover:shadow-xl transition-all duration-300 backdrop-blur-sm">
        <CardHeader
          className={cn(
            cardPaddingClass,
            "border-b-2 border-slate-200 bg-gradient-to-r from-slate-100 to-slate-50"
          )}
        >
        ...
        <CardContent className={cn(cardContentPaddingClass, compact ? "space-y-1" : "space-y-1.5")}>
          {quickActions.map((action, index) => (
            <V28Button
              key={index}
              variant={
                action.variant === "quick-action-primary" || action.variant === "secondary"
                  ? "primary"
                  : "secondary"
              }
              size="sm"
      ...
      <Card className="border-slate-300 bg-white/95 shadow-lg hover:shadow-xl transition-all duration-300 backdrop-blur-sm">
        ...
        <CardContent className={cardContentPaddingClass}>{contextWidget.content}</CardContent>
      </Card>
    </div>
  );
}
