...
export function TopCustomersWidget() {
  ...
  const topCustomers = useMemo<TopCustomer[]>(() => {
    ...
    bookings
      .filter(b => !b.archived && b.customer_id && b.price && b.price > 0 && b.payment_status === 'paid')
      .forEach(booking => {
    ...
    return (Array.from(customerStats.entries())
      .map(([customerId, stats]) => {
        ...
        return {
          id: customerId,
          name: `${customer.first_name || ''} ${customer.last_name || ''}`.trim() || customer.company_name || 'Unbekannt',
