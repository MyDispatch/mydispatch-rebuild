    ...
    >
      <div className="flex flex-col sm:flex-row items-start gap-4 sm:gap-5">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-xl md:text-2xl font-semibold mb-3 text-slate-900">{title}</h3>

          {price && (
            <div className="mb-4">
              <span className="text-2xl md:text-3xl font-bold text-slate-900">{price}</span>
              {priceLabel && <span className="text-sm ml-2 text-slate-600">{priceLabel}</span>}
            </div>
          )}

          <p className="text-sm md:text-base leading-relaxed mb-5 text-slate-600">{description}</p>
