...
export function Breadcrumbs() {
  ...
  try {
    ...
    logger.warn('[Breadcrumbs] Router context not available, skipping render', { component: 'Breadcrumbs' });
    return null;
  }

  // DEFENSIVE PROGRAMMING: Fallback wenn location undefined
  if (!location) {
    logger.warn('[Breadcrumbs] Location is undefined, skipping render', { component: 'Breadcrumbs' });
    return null;
  }
  ...
  return (
    <nav className="flex items-center gap-2 text-sm text-muted-foreground mb-6" aria-label="Breadcrumb">{/* pt-6 entfernt, da MainLayout bereits pt-24 hat */}
      <Link
        to="/dashboard"
      ...
      {pathnames.map((pathname, index) => {
        ...
        return (
          <div key={routeTo} className="flex items-center gap-2">
            ...
            {isLast ? (
              <span className={cn("font-medium text-foreground", "truncate max-w-[200px] sm:max-w-none")}>
                {displayName}
              </span>
            ) : (
              <Link
