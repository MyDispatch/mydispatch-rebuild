...
export function IntelligentAIChat({
  isPublicLanding = false,
  companyData,
}: IntelligentAIChatProps = {}) {
  ...
  try {
    ...
    logger.warn("[IntelligentAIChat] Router context not available", {
      component: "IntelligentAIChat",
    });
    ...
    try {
      const context: any = {
        ...
        user: {
          ...
          email: user?.email,
          role: permissions.canAccessMasterDashboard
  ...
  const getPageContext = () => {
    ...
    const contexts: Record<string, { title: string; description: string }> = {
      "/": {
        title: "Dashboard",
        description: "Übersicht über aktuelle Aufträge, verfügbare Fahrer und Fahrzeugstatus",
      },
      "/auftraege": {
        title: "Auftragsverwaltung",
        description: "Verwaltung und Disposition von Buchungen und Fahrten",
      },
      "/fahrer": {
        title: "Fahrerverwaltung",
        description: "Verwaltung der Fahrer, Lizenzen und Schichtzuweisungen",
      },
      "/fahrzeuge": {
        title: "Fahrzeugverwaltung",
        description: "Verwaltung der Fahrzeugflotte, Konzessionen und Wartungen",
      },
      "/kunden": {
        title: "Kundenverwaltung",
        description: "Verwaltung von Privat- und Geschäftskunden",
      },
      "/partner": {
        title: "Partner-Netzwerk",
        description: "Kooperationen mit anderen Taxi- und Mietwagenunternehmen",
      },
      "/schichtzettel": {
        title: "Schichtzettel",
        description: "Dokumentation von Schichten und Einnahmen (P-Schein-konform)",
      },
      "/rechnungen": {
        title: "Rechnungsverwaltung",
        description: "Erstellung und Verwaltung von Rechnungen",
      },
      "/statistiken": {
        title: "Statistiken & Reporting",
        description: "Auswertungen und Analysen zur Unternehmensperformance",
      },
      "/master": {
        title: "Master-Dashboard",
        description: "Verwaltung aller Unternehmen, Terminierung und Analytics",
      },
    };
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content:
            "Entschuldigung, es ist ein Fehler aufgetreten. Bitte versuchen Sie es erneut oder kontaktieren Sie den Support.",
          timestamp: new Date(),
        },
      ]);
      toast({
  ...
  if (chatMode === "minimized") {
    return (
      <V28Button
        ...
        aria-label={
          isPublicLanding
            ? `${companyData?.name || "Service"}-Assistent öffnen`
            : "MyDispatch AI-Assistent öffnen"
        }
      >
    ...
    >
      ...
      >
        <div className="flex-1 min-w-0 pr-2">
          ...
          <p
            className={cn(
              "ai-chat-header__subtitle truncate",
              isMobile ? "text-[11px] mt-0.5" : "text-xs mt-1"
            )}
          >
            {isPublicLanding
      ...
      {!isPublicLanding && (
        ...
        >
          <span className="font-medium">Hinweis:</span> AI-System MyDispatch AI | Zweck: Support &
          Analyse |{" "}
          <a
            href="/datenschutz"
            target="_blank"
          ...
          <div className={cn("text-center space-y-3", isMobile ? "mt-4" : "mt-8")}>
            ...
            >
              {isPublicLanding ? (
                ...
                <span
                  className={cn("font-bold text-foreground", isMobile ? "text-xl" : "text-2xl")}
                >
                  AI
                </span>
              )}
            ...
            {!isPublicLanding && contextData && (
              ...
              >
                ...
                <div className={cn("space-y-1", isMobile ? "text-xs" : "text-sm")}>
                  <p>
                    <span className="font-medium">Seite:</span> {contextData.page?.title}
                  </p>
                  <p>
                    <span className="font-medium">Unternehmen:</span> {contextData.company?.name}
                  </p>
                  <p>
                    <span className="font-medium">Tarif:</span> {contextData.company?.tariff}
                  </p>
                </div>
              </div>
            )}
            ...
            <div className={cn("max-w-md mx-auto space-y-2", isMobile ? "mt-4" : "mt-6")}>
              ...
              {(isPublicLanding ? getLandingSuggestedQuestions() : getAppSuggestedQuestions()).map(
                (question, idx) => (
                  <V28Button
                    key={idx}
                    variant="secondary"
            ...
            {isPublicLanding && companyData?.phone && (
              <div className={cn("border-t", isMobile ? "mt-4 pt-4" : "mt-6 pt-6")}>
                <p
                  className={cn("text-muted-foreground mb-2", isMobile ? "text-[10px]" : "text-xs")}
                >
          ...
          >
            {message.role === "assistant" && (
              ...
              >
                {isPublicLanding ? (
                  <Car
                    className={cn("text-primary-foreground", isMobile ? "h-3.5 w-3.5" : "h-4 w-4")}
                  />
                ) : (
                  <span
                    className={cn(
            ...
            >
              ...
              />
              {message.timestamp && (
                <div className={cn("opacity-60 mt-1", isMobile ? "text-[10px]" : "text-xs")}>
          ...
          <div className="flex gap-2 justify-start">
            ...
            <div className={cn("bg-muted rounded-2xl", isMobile ? "px-3 py-2" : "px-4 py-3")}>
              <div className="flex gap-1">
                <span
                  className={cn(
                    "ai-chat-loading-dot ai-chat-loading-dot--0",
                    isMobile ? "w-1.5 h-1.5" : "w-2 h-2"
                  )}
                />
                <span
                  className={cn(
                    "ai-chat-loading-dot ai-chat-loading-dot--1",
                    isMobile ? "w-1.5 h-1.5" : "w-2 h-2"
                  )}
                />
                <span
                  className={cn(
                    "ai-chat-loading-dot ai-chat-loading-dot--2",
                    isMobile ? "w-1.5 h-1.5" : "w-2 h-2"
                  )}
                />
              </div>
      ...
      <div className={cn("ai-chat-input-container", isMobile ? "p-2.5" : "p-4")}>
        <div className={cn("flex gap-2", isMobile && "items-end")}>
          ...
          <V28Button
            ...
            size="sm"
            variant="primary"
            className={cn("ai-chat-send-btn", isMobile ? "h-11 w-11" : "h-11 w-11")}
            aria-label="Senden"
          >
