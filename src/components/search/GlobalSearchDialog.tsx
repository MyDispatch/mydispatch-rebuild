...
export function GlobalSearchDialog() {
  ...
  useEffect(() => {
    try {
      ...
      logger.error('[GlobalSearchDialog] Failed to load recent searches', error as Error, { component: 'GlobalSearchDialog' });
    }
  }, [open]);
  ...
  const saveRecentSearch = useCallback((query: string) => {
    ...
    try {
      ...
      logger.error('[GlobalSearchDialog] Failed to save recent search', error as Error, { component: 'GlobalSearchDialog' });
    }
  }, []);
  ...
  const performSearch = useCallback(async (query: string) => {
    ...
    try {
      ...
      const { data: drivers, error: driversError } = await supabase
        ...
        .eq('company_id', profile.company_id)
        .eq('archived', false)
        .or(`first_name.ilike.%${searchLower}%,last_name.ilike.%${searchLower}%,license_number.ilike.%${searchLower}%`)
        .limit(5);
