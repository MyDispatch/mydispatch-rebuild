  ...
  return (
    <div className={cn("space-y-2", className)}>
      ...
      <div className="space-y-1">
        ...
        <Progress value={strength.score} className="h-2" indicatorClassName={config.bgColor} />
      </div>
      ...
      <div className="space-y-1 pt-2 border-t">
        ...
        <RequirementItem met={password.length >= 8} label="Mindestens 8 Zeichen" />
        <RequirementItem
          met={/[A-Z]/.test(password) && /[a-z]/.test(password)}
          label="Groß- und Kleinbuchstaben"
        />
        <RequirementItem met={/\d/.test(password)} label="Mindestens eine Zahl" />
        <RequirementItem
          met={/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>/?]/.test(password)}
          label="Mindestens ein Sonderzeichen"
        />
        <RequirementItem met={!strength.isCommon} label="Kein häufiges Passwort" />
      </div>
    </div>
  );
}
...
function RequirementItem({ met, label }: { met: boolean; label: string }) {
  return (
    <div className="flex items-center gap-2 text-xs">
      ...
      <span className={cn(met ? "text-status-success" : "text-muted-foreground")}>{label}</span>
    </div>
  );
}
