...
export function DashboardRenderer({ 
  ...
  const demoData = useMemo(() => ({
    ...
    vehicles: liveData ? vehicles.filter(v => !v.archived && v.status === 'available').length : 18
        ...
        <div className="p-8 space-y-8">
          <V28DashboardSection background="white" className="py-8">
            ...
            <DataGrid columns={{ mobile: 1, tablet: 2, desktop: 4 }} gap="lg">
              ...
              <V28StatCard
                label="Aktive Fahrer"
                value={demoData.drivers}
                icon={Users}
              />
              <V28StatCard
                label="Verfügbare Fahrzeuge"
                value={demoData.vehicles}
                icon={Car}
              />
            </DataGrid>
          </V28DashboardSection>
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="space-y-2">
            {[1, 2, 3, 4, 5].map(i => (
              <div 
                key={i} 
                className="h-16 bg-white border border-slate-200 rounded-lg"
              />
            ))}
          </div>
        </div>
