...
export function SafeIcon({
  ...
  if (import.meta.env.DEV && !isValidIconColor(color)) {
    logger.warn(`SafeIcon: Unerlaubte Farbe "${color}"`, {
  ...
  if (import.meta.env.DEV && className) {
    ...
    const hasForbiddenColor = forbiddenColors.some((forbidden) => className.includes(forbidden));

    if (hasForbiddenColor) {
      logger.warn(`SafeIcon: Verbotene Farbe in className`, {
        className,
        reason: "Ampelfarben verboten auf Icons",
      });
    }
  }
  ...
  return (
    <Icon
      className={cn(SIZE_MAP[size], mappedColor, onClick && "cursor-pointer", className)}
      onClick={onClick}
      aria-hidden={!onClick}
    />
  );
