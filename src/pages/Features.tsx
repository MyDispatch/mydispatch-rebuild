...
import {
  MapPin, Users, Car, FileText, BarChart3, Calendar,
  MessageSquare, Smartphone, Shield, Zap, Clock, CheckCircle2, ArrowRight
} from 'lucide-react';
...
const features = [
  {
    ...
    description: 'Live-Standortverfolgung aller Fahrzeuge mit automatischer Routenoptimierung und Traffic-Daten.',
    tariff: 'Alle Tarife',
  ...
  },
  {
    ...
    description: 'Automatische Auftragszuweisung basierend auf Verfügbarkeit, Standort und Fahrzeugtyp.',
    tariff: 'Alle Tarife',
  ...
  },
  {
    ...
    description: 'Zentrale Datenbank mit Buchungshistorie, Präferenzen und automatischer Rechnungserstellung.',
    tariff: 'Alle Tarife',
  ...
  },
  {
    icon: Smartphone,
    title: 'Mobile Apps',
    description: 'Native iOS/Android Apps für Fahrer mit Offline-Modus und Push-Benachrichtigungen.',
    tariff: 'Alle Tarife',
  ...
  },
  {
    ...
    description: 'RESTful API für Anbindung externer Systeme (Buchhaltung, Zahlungsanbieter, etc.).',
    tariff: 'Business+',
...
const faqItems = [
  {
    ...
    answer: 'Der Starter-Tarif umfasst alle Basis-Features: Disposition, Kunden-/Fahrerverwaltung, Auftragsverwaltung, Rechnungserstellung und Landingpage für bis zu 3 Fahrer/Fahrzeuge.'
  },
  {
    question: 'Was bietet der Business-Tarif zusätzlich?',
    answer: 'Business erweitert Starter um: Beliebig viele Fahrer/Fahrzeuge, Partner-Management, Live-Traffic, Statistiken, Kunden-Portal, Buchungswidget und API-Zugang.'
  },
  {
    question: 'Kann ich Features einzeln dazubuchen?',
    answer: 'Nein, Features sind in Tarifpaketen gebündelt. Sie können jedoch jederzeit zwischen den Tarifen wechseln (Upgrade/Downgrade).'
  },
  {
    question: 'Gibt es eine mobile App?',
    answer: 'Ja! MyDispatch ist als responsive Web-App konzipiert und funktioniert auf allen Geräten. Native mobile Apps sind für 2025 geplant.'
  },
  {
    question: 'Werden regelmäßig neue Features veröffentlicht?',
    answer: 'Ja, wir veröffentlichen alle 4-6 Wochen Updates mit neuen Features und Verbesserungen. Updates sind für alle Tarife inklusive.'
  }
];
  ...
  return (
    <MarketingLayout currentPage="features">
      <SEOHead
        ...
        keywords={['MyDispatch Features', 'Taxi Software Funktionen', 'Dispositionssoftware Features', 'GPS Tracking', 'Fuhrparkverwaltung']}
      />
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-slate-200">
                <th className="text-left py-4 px-4 font-sans text-sm font-semibold text-slate-900">Feature</th>
                <th className="text-center py-4 px-4 font-sans text-sm font-semibold text-slate-900">Starter</th>
                <th className="text-center py-4 px-4 font-sans text-sm font-semibold text-slate-900">Business</th>
                <th className="text-center py-4 px-4 font-sans text-sm font-semibold text-slate-900">Enterprise</th>
              </tr>
            </thead>
            <tbody>
              {features.map((feature, idx) => {
                const inStarter = feature.tariff === 'Alle Tarife';
                const inBusiness = feature.tariff === 'Alle Tarife' || feature.tariff === 'Business+';
                ...
                return (
                  <tr key={idx} className="border-b border-slate-100">
                    ...
                    <td className="text-center py-3 px-4">
                      {inStarter ? <CheckCircle2 className="h-5 w-5 text-slate-700 mx-auto" /> : <span className="text-slate-400">—</span>}
                    </td>
                    <td className="text-center py-3 px-4">
                      {inBusiness ? <CheckCircle2 className="h-5 w-5 text-slate-700 mx-auto" /> : <span className="text-slate-400">—</span>}
                    </td>
                    <td className="text-center py-3 px-4">
                      {inEnterprise ? <CheckCircle2 className="h-5 w-5 text-slate-700 mx-auto" /> : <span className="text-slate-400">—</span>}
                    </td>
                  </tr>
                );
              })}
