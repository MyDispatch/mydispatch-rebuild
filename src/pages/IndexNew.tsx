...
export default function IndexNew() {
  ...
  const revenueCash = todayBookings
    .filter((b) => b.payment_method === "Barzahlung" || b.payment_method === "cash")
    .reduce((sum, b) => sum + (b.price || 0), 0);
  const revenueInvoice = todayBookings
    .filter((b) => b.payment_method === "Rechnung" || b.payment_method === "invoice")
    .reduce((sum, b) => sum + (b.price || 0), 0);
  const revenueCard = todayBookings
    .filter((b) => b.payment_method === "Kartenzahlung" || b.payment_method === "card")
    .reduce((sum, b) => sum + (b.price || 0), 0);
      ...
      component: (
        <ResourceStatusWidget
          availableDrivers={drivers
            .filter((d) => !d.archived && d.shift_status === "available")
            .slice(0, 3)
            .map((d) => ({
              id: d.id,
          ...
          busyDrivers={drivers
            .filter((d) => !d.archived && d.shift_status === "busy")
            .slice(0, 3)
            .map((d) => ({
              id: d.id,
              first_name: d.first_name,
          ...
          breakdown={[
            {
              label: "Bar",
              value: revenueCash,
              percentage: todayTotal > 0 ? Math.round((revenueCash / todayTotal) * 100) : 0,
              color: "hsl(var(--chart-primary))",
            },
            {
              label: "Rechnung",
              value: revenueInvoice,
              percentage: todayTotal > 0 ? Math.round((revenueInvoice / todayTotal) * 100) : 0,
              color: "hsl(var(--primary))",
            },
            {
              label: "Karte",
              value: revenueCard,
              percentage: todayTotal > 0 ? Math.round((revenueCard / todayTotal) * 100) : 0,
        ...
        <Card className="border-primary/20 bg-primary/5">
          <CardContent className="pt-6">
            <div className="text-center py-6">
              ...
              <V28Button size="sm" onClick={() => navigate("/pricing")} variant="primary">
                Jetzt upgraden
              </V28Button>
            </div>
          </CardContent>
                ...
                <Card className="border-primary/20 bg-primary/5">
                  <CardContent className="pt-6">
                    <div className="flex items-center justify-between">
                      ...
                      <V28Button onClick={() => navigate("/pricing")} variant="primary">
                        Tarife ansehen
                      </V28Button>
                    </div>
                  </CardContent>
