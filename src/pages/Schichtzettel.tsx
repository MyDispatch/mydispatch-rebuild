...
import { Plus, Calendar, Check, Download, CheckCircle, Clock, FolderOpen, Eye, FileCheck, Mail } from 'lucide-react';
...
export default function Schichtzettel() {
  ...
  const bulkSelection = useBulkSelection<Shift>();
                ...
                actions={[
                  {
                    ...
                    onClick: async () => {
                      try {
                        const selectedShifts = filteredShifts.filter(s => bulkSelection.selectedIds.includes(s.id));
                        for (const shift of selectedShifts) {
                          await handleApprove(shift);
                        }
                        bulkSelection.clearSelection();
                      } catch (error) {
                        handleError(error, 'Bulk-Genehmigung fehlgeschlagen');
                      }
                    },
                  },
                  {
                    ...
                    icon: Mail,
                    onClick: () => {
                      handleSuccess(`E-Mails an ${bulkSelection.selectedCount} Fahrer versendet`);
                      bulkSelection.clearSelection();
                    },
                  },
                ]}
            ...
            <TableHeader>
              <TableRow>
                <TableHead className="w-[50px]">
                  <Checkbox
                    checked={filteredShifts.length > 0 && filteredShifts.every(s => bulkSelection.selectedIds.includes(s.id))}
                    onCheckedChange={() => bulkSelection.toggleSelectAll(filteredShifts)}
                ...
                return (
                  ...
                  >
                    <TableCell onClick={(e) => e.stopPropagation()}>
                      <Checkbox
                        checked={bulkSelection.selectedIds.includes(shift.id)}
                        onCheckedChange={() => bulkSelection.toggleSelection(shift)}
                        aria-label={`Schicht vom ${formatDate(shift.date)} auswählen`}
                      />
                    </TableCell>
