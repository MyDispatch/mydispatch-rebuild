...
const Rechnungen = () => {
  ...
  const handleBulkReminder = async () => {
    try {
      ...
      handleSuccess(
        `${data.sent} Zahlungserinnerungen erfolgreich versendet${data.failed > 0 ? ` (${data.failed} fehlgeschlagen)` : ""}`
      );
      bulkSelection.clearSelection();
    } catch (error) {
      ...
      <>
        ...
        >
          <StandardPageLayout
            ...
            searchPlaceholder={
              currentTab === "rechnungen" ? "Rechnungen durchsuchen..." : "Angebote durchsuchen..."
            }
          >
  ...
  const renderInvoiceTable = (filteredInvoices: Invoice[]) => {
    ...
    const isAllSelected =
      ...
      filteredInvoices.every((inv) => bulkSelection.selectedIds.includes(inv.id));
                    ...
                    <EmptyState
                      ...
                      description={
                        searchTerm
                          ...
                          : "Rechnungen werden automatisch aus abgeschlossenen Aufträgen generiert"
                      }
  ...
  return (
    <>
      <StandardPageLayout
        ...
        onCreateNew={() =>
          currentTab === "rechnungen"
            ? setCreateDialogOpen(true)
            : navigate("/auftraege", { state: { createOffer: true } })
        }
      ...
      >
        ...
        <Tabs value={currentTab} onValueChange={(value) => setSearchParams({ tab: value })}>
          ...
          <TabsContent value="rechnungen">{renderInvoiceTable(allInvoices)}</TabsContent>
              ...
              <EmptyState
                ...
                description={
                  searchTerm
                    ...
                    : "Erstellen Sie Angebote in der Aufträge-Seite"
                }
                      ...
                      .map((offer) => (
                        <TableRow key={offer.id}>
                          ...
                          <TableCell className="hidden lg:table-cell">
                            {format(new Date(offer.pickup_time), "dd.MM.yyyy HH:mm", {
                              locale: de,
                            })}
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell className="text-right">
                            <div className="flex gap-2 justify-end">
                              <V28Button
                                ...
                                variant="secondary"
                                onClick={() =>
                                  navigate("/auftraege", { state: { viewOffer: offer.id } })
                                }
                              >
                                <Eye className="h-4 w-4" />
                              </V28Button>
                  ...
                  <RelatedEntityCard
                    ...
                    actions={[
                      ...(invoiceCustomer.phone
                        ? [getStandardActions.phone(invoiceCustomer.phone)]
                        : []),
                      ...(invoiceCustomer.email
                        ? [getStandardActions.email(invoiceCustomer.email)]
                        : []),
                    ]}
                  />
                )}
