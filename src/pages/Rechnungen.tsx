...
import { FileText, Download, Mail, Send, Eye, Euro, BookOpen, Plus } from 'lucide-react';
...
const Rechnungen = () => {
  ...
  const allInvoices = useMemo(
    () => invoices.filter(invoice => {
      // Search filter
      const matchesSearch = getCustomerName(invoice.customer_id).toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const loadInvoiceRelatedData = async (invoice: Invoice) => {
    ...
    try {
      ...
      if (customerData) {
        setInvoiceCustomer(customerData as unknown as Customer);
      } else {
  ...
  // Export handler for actions prop
  const handleExport = async () => {
    await handleBulkPDFExport();
  };
  ...
  return (
    <>
      <StandardPageLayout
          ...
          actions={[
            {
              label: currentTab === 'rechnungen' ? "Neue Rechnung" : "Neues Angebot",
              onClick: () => currentTab === 'rechnungen' ? setCreateDialogOpen(true) : navigate('/auftraege', { state: { createOffer: true } }),
              icon: Plus,
