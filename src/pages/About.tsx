  ...
  const values = [
    {
      ...
      title: "Branchennähe",
      description:
        "Wir kennen die Taxi-Branche aus eigener Erfahrung und verstehen Ihre Herausforderungen.",
    },
    {
      icon: Target,
      title: "Qualität",
      description:
        "Höchste Ansprüche an Code-Qualität, Sicherheit und Datenschutz (Made in Germany).",
    },
    {
      icon: Users,
      title: "Partnerschaft",
      description:
        "Sie sind nicht nur Kunde - Sie sind Partner. Ihr Feedback gestaltet unser Produkt.",
    },
  ];
    ...
    <MarketingLayout currentPage="about">
      <SEOHead
        ...
        keywords={[
          "MyDispatch Team",
          "Über MyDispatch",
          "Taxi Software Entwickler",
          "Made in Germany",
          "MyDispatch Geschichte",
        ]}
      />
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div className="text-3xl font-bold text-slate-900">{event.year}</div>
                </div>
                <h4 className="font-sans text-lg font-semibold text-slate-900 mb-2">
                  {event.title}
                </h4>
                <p className="font-sans text-sm text-slate-600">{event.description}</p>
              </V28MarketingCard>
            );
          })}
        </div>
      ...
      {/* Values Section */}
      <V28MarketingSection background="white" title="Unsere Werte" description="Was uns antreibt">
          ...
          {values.map((value, index) => {
            ...
            return (
              ...
              >
                ...
                <p className="font-sans text-sm text-slate-600">{value.description}</p>
              </V28MarketingCard>
            );
          })}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {teamMembers.map((member, index) => (
            ...
            >
              ...
              <h4 className="font-sans text-lg font-semibold text-slate-900 mb-2">{member.name}</h4>
              <Badge className="mb-3 bg-slate-100 text-slate-700 hover:bg-slate-200">
                {member.role}
              </Badge>
              <p className="font-sans text-sm text-slate-600">{member.description}</p>
            </V28MarketingCard>
          ))}
        </div>
      </V28MarketingSection>
      ...
      <V28MarketingSection background="white">
        <V28MarketingCard className="bg-primary text-primary-foreground text-center">
          <h2 className="text-2xl md:text-3xl font-bold mb-4">Bereit, durchzustarten?</h2>
          <p className="max-w-2xl mx-auto text-base md:text-lg opacity-90 mb-6">
            Werden Sie Teil der MyDispatch-Familie und profitieren Sie von 15 Jahren
            Branchenerfahrung.
          </p>
