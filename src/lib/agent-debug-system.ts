...
export interface DebugError {
  ...
  autoFixable: boolean;
  solution?: string;
  category: 'design-system' | 'mobile-first' | 'accessibility' | 'security' | 
            'performance' | 'code-quality' | 'css' | 'api-backend' | 'runtime' | 
            'functionality' | 'state-management' | 'lifecycle' | 'config-drift' |
            'async-errors' | 'ui-validation' | 'security-audit' | 'system-interactions';
  priority?: number;
  tags?: string[];
  timestamp?: string;
    ...
    lines.forEach((line, index) => {
      ...
      if (/accent/.test(line) && !line.includes('//') && !line.includes('/*')) {
        errors.push({
          ...
          severity: 'critical',
          message: 'VERBOTEN: accent color detected - accent is completely removed from design system',
          line: index + 1,
          context: line.trim(),
          autoFixable: true,
      ...
      const directColors = ['text-white', 'bg-white', 'text-black', 'bg-black', 'border-white', 'border-black'];
      directColors.forEach(color => {
        if (line.includes(color) && !line.includes('//') && !line.includes('/*')) {
          errors.push({
            ...
            context: line.trim(),
            autoFixable: true,
            solution: 'Use semantic tokens: text-foreground, bg-background, text-muted-foreground, bg-card, etc.',
            category: 'design-system',
            priority: 85,
            tags: ['design-system', 'color']
          });
      ...
      if (line.includes('<') && /(?:Icon|Lucide)/.test(line)) {
        if (!line.includes('text-foreground') && !line.includes('text-muted-foreground') && 
...
class SecurityScanner {
  static scan(filePath: string, content: string): DebugError[] {
    ...
    lines.forEach((line, index) => {
      ...
      if (line.includes('auth.users') && (filePath.includes('migration') || filePath.includes('.sql'))) {
        errors.push({
      ...
      if (line.includes('CREATE POLICY') && line.includes('SELECT')) {
        ...
        if (policyName && (policyName.toLowerCase().includes('customer') || policyName.toLowerCase().includes('view'))) {
          errors.push({
...
class CodeQualityScanner {
  static scan(filePath: string, content: string): DebugError[] {
    ...
    lines.forEach((line, index) => {
      // Inline formatters
      if (/(toLocaleString|toFixed|new Date.*format)/.test(line) && 
  ...
  static scan(filePath: string, content: string): DebugError[] {
    ...
    lines.forEach((line, index) => {
      ...
      if (line.includes('<form') || line.includes('onSubmit')) {
        ...
        if (!formBlock.includes('zod') && !formBlock.includes('yup') && !formBlock.includes('validate')) {
          errors.push({
  ...
  static scan(filePath: string, content: string): DebugError[] {
    ...
    lines.forEach((line, index) => {
      if (line.includes('useEffect')) {
        ...
        if ((effectBlock.includes('addEventListener') || 
...
class SystemInteractionsScanner {
  static scan(filePath: string, content: string): DebugError[] {
    ...
    lines.forEach((line, index) => {
      if ((line.includes('fetch(') || line.includes('axios.') || line.includes('supabase')) &&
          !line.includes('trackError') && !line.includes('console.error')) {
...
class UIValidationScanner {
  static scan(filePath: string, content: string): DebugError[] {
    ...
    lines.forEach((line, index) => {
      if (/className=["'].*\b(w-\[\d+px\]|h-\[\d+px\]|text-\[\d+px\]|p-\[\d+px\]|m-\[\d+px\])/.test(line)) {
        errors.push({
      ...
      if (line.includes('export default') && content.includes('useState') && 
...
class ConfigDriftScanner {
  static scan(filePath: string, content: string): DebugError[] {
    ...
    lines.forEach((line, index) => {
      if (/(import\.meta\.env\.|process\.env\.)/.test(line) && 
          !content.includes('if (!')) {
...
export class AgentDebugSystem {
  ...
  public scanFile(filePath: string, content: string): DebugError[] {
    ...
    for (const scanner of this.scanners) {
      try {
        ...
        const errorsWithFile = errors.map(err => ({ ...err, file: filePath, title: err.message }));
  ...
  public async scanFiles(files: Array<{ path: string; content: string }>): Promise<PageScanResult> {
    ...
    const errorsByCategory = allErrors.reduce((acc, err) => {
  ...
  public generateReport(errors: DebugError[]): string {
    ...
    errors.slice(0, 10).forEach((err, idx) => {
      const icon = err.severity === 'critical' ? '🔴' : err.severity === 'high' ? '🟠' : err.severity === 'medium' ? '🟡' : '🟢';
