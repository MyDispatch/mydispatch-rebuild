...
export const KNOWN_ISSUES: Array<{
  ...
  // DELETE statt Archiving
  {
    ...
    getPrompt: (filePath) => 
      `Kritisch: Ersetze .delete() mit Archiving-System in ${filePath}`
  },
  ...
  // Hex-Farben statt Design-Tokens
  {
    ...
    getPrompt: (filePath, match) => 
      `Ersetze Hex-Farbe ${match[0]} mit Design-Token in ${filePath}`
  },
  ...
  // Inline-Formatierung
  {
    ...
    getPrompt: (filePath) => 
      `Refactore Inline-Formatierung in ${filePath} zu format-utils`
  },
  ...
  // Console.logs in Production
  {
    ...
    getPrompt: (filePath) => 
      `Entferne/ersetze console.logs in ${filePath} mit proper logging`
  },
  ...
  // Unvalidated User Inputs
  {
    ...
    getPrompt: (filePath) => 
      `Füge Input-Validierung hinzu in ${filePath}`
  },
  ...
  // Missing Error Boundaries
  {
    ...
    getPrompt: (filePath) => 
      `Prüfe ob ${filePath} Error-Boundary benötigt`
  }
];
...
function calculatePriority(
  ...
  return Math.min(10, Math.round(
    (severityWeight[severity] + categoryWeight[category]) / 2
  ));
}
