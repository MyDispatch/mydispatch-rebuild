...
export function getBookingColumns<T = any>(): ColumnDef<T>[] {
  return [
    // PFLICHT: Auftragsnummer
    {
      ...
      cell: (item: any) => (
        <span className="font-medium text-foreground">{item.booking_number || "-"}</span>
      ),
      sortable: true,
    ...
    // PFLICHT: Auftragseingangsdatum/-zeit (§ 51 PBefG)
    {
      ...
      cell: (item: any) => (
        <div className="text-sm">
          <div className="font-medium text-foreground">{formatDateTime(item.created_at)}</div>
          <div className="text-xs text-muted-foreground">Rechtl. Zeitstempel</div>
        </div>
      ),
      sortable: true,
    ...
    // PFLICHT: Fahrpreis
    {
      ...
      cell: (item: any) => (
        <span className="font-medium text-foreground">{formatCurrency(item.price || 0)}</span>
      ),
      sortable: true,
    },
    ...
    // PFLICHT: Status
    {
      ...
      cell: (item: any) => (
        <StatusIndicator type={getBookingStatusType(item.status)} label={item.status} />
      ),
    },
  ];
}
  ...
  return [
    ...
    // PFLICHT: Rechnungsdatum (§ 14 UStG)
    {
      ...
      cell: (item: any) => (
        <div className="text-sm">
          <div className="font-medium text-foreground">{formatDateTime(item.created_at)}</div>
          <div className="text-xs text-muted-foreground">§ 14 UStG</div>
        </div>
      ),
      sortable: true,
      mandatory: true,
    ...
    // PFLICHT: Bruttobetrag
    {
      ...
      cell: (item: any) => (
        <div className="text-sm">
          <div className="font-medium text-foreground">{formatCurrency(item.total || 0)}</div>
          <div className="text-xs text-muted-foreground">inkl. {item.tax_rate || 19}% MwSt.</div>
        </div>
      ),
      sortable: true,
    },
...
export function getCustomerColumns<T = any>(): ColumnDef<T>[] {
  return [
    ...
    // PFLICHT: Erfassungsdatum (DSGVO Art. 30)
    {
      ...
      cell: (item: any) => (
        <div className="text-sm">
          <div className="text-foreground">{formatDateTime(item.created_at)}</div>
          <div className="text-xs text-muted-foreground">DSGVO Art. 30</div>
        </div>
      ),
      sortable: true,
      mandatory: true,
    ...
    // Kontaktdaten
    {
      ...
      cell: (item: any) => (
        <div className="text-xs">
          {item.email && <div className="text-foreground">{item.email}</div>}
    ...
    // Anzahl Fahrten
    {
      ...
      cell: (item: any) => (
        <span className="text-sm text-foreground">{item.total_bookings || 0}</span>
      ),
      sortable: true,
    },
  ];
      ...
      cell: (item: any) => (
        <div className="text-sm">
          ...
          {item.employee_number && (
            <div className="text-xs text-muted-foreground">MA-Nr: {item.employee_number}</div>
          )}
        </div>
      ),
      sortable: true,
      ...
      cell: (item: any) => {
        ...
        return (
          <div className="text-sm">
            ...
            {isExpired && <div className="text-xs text-status-error font-medium">ABGELAUFEN!</div>}
          </div>
        );
      },
      mandatory: true, // NIEMALS verstecken!
...
export function getVehicleColumns<T = any>(): ColumnDef<T>[] {
  return [
    // Kennzeichen
    {
      ...
      cell: (item: any) => (
        <span className="font-mono font-medium text-foreground">{item.license_plate}</span>
      ),
      sortable: true,
    },
