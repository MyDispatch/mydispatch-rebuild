 ...
 */
export function filterByDateRange<T extends { created_at: string }>(data: T[], days: number): T[] {
 ...
 */
export function groupByDate<T extends { created_at: string }>(data: T[]): Record<string, T[]> {
  return data.reduce(
    (acc, item) => {
...
export class DashboardStatsCalculator {
  ...
  static customers(customers: any[]) {
    ...
    const openInvoices = customers.filter((c) => !c.archived && c.outstanding_balance > 0).length;
  ...
  static bookings(bookings: any[]) {
    ...
    const todayBookings = bookings.filter((b) => !b.archived && b.pickup_time?.startsWith(today));
    const todayCount = todayBookings.length;
    const todayRevenue = calculateSum(todayBookings.map((b) => b.price || 0));

    const thisMonth = filterByDateRange(bookings, 30);
    const lastMonth = filterByDateRange(bookings, 60).filter((b) => !thisMonth.includes(b));
