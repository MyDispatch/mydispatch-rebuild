...
export async function postCommitIndexing(changedFiles: string[]): Promise<void> {
  const highCriticalityFiles = changedFiles.filter(
    f => determineCriticality(f) === 'high'
  );
  const mediumCriticalityFiles = changedFiles.filter(
    f => determineCriticality(f) === 'medium'
  );
