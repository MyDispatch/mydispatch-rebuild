...
class ErrorToChatPipeline {
  ...
  generateErrorReport(error: Error | unknown): ErrorReport | null {
    ...
    if (recentErrors.length === 0) {
      if (import.meta.env.DEV) {
        logger.warn('[ErrorToChatPipeline] No tracked errors found', { component: 'ErrorToChatPipeline' });
      }
      return null;
    }
  ...
  formatForChat(report: ErrorReport): string {
    const lines = [
      ...
      ...report.userActions.slice(-5).map((action, i) => 
        `${i + 1}. ${action.action} → ${action.target}`
      ),
      '',
      '**Geräte-Info:**',
    ...
    if (report.error.stack) {
      lines.push('', '**Stack-Trace:**', '```', report.error.stack.split('\n').slice(0, 5).join('\n'), '```');
    }
  ...
  async sendErrorToChat(error: Error | unknown): Promise<boolean> {
    try {
      ...
      if (import.meta.env.DEV) {
        logger.debug('[ErrorToChatPipeline] Report generiert', { component: 'ErrorToChatPipeline', formattedReport });
      }
      ...
      if (navigator.clipboard) {
        ...
        if (import.meta.env.DEV) {
          logger.debug('[ErrorToChatPipeline] Report in Zwischenablage kopiert', { component: 'ErrorToChatPipeline' });
        }
      }
      ...
      if (import.meta.env.DEV) {
        logger.error('[ErrorToChatPipeline] Fehler beim Senden', err as Error, { component: 'ErrorToChatPipeline' });
      }
      return false;
    }
  }
