...
export const ENTERPRISE_TARIFF: TariffDefinition = {
  ...
  features: [
    ...
    {
      id: "unlimited_users",
      name: "Keine Begrenzung bei Benutzern",
      included: true,
      module: "users",
    },
...
// ==================================================================================
export const ALL_TARIFFS: TariffDefinition[] = [STARTER_TARIFF, BUSINESS_TARIFF, ENTERPRISE_TARIFF];
...
export function getTariffById(
  id: "starter" | "business" | "enterprise"
): TariffDefinition | undefined {
  return ALL_TARIFFS.find((t) => t.id === id);
}

export function getTariffByProductId(
  productId: string | null | undefined
): TariffDefinition | undefined {
  if (!productId) return undefined;
  return ALL_TARIFFS.find((t) => (t.stripeProductIds as readonly string[]).includes(productId));
}
...
export const ADDON_FLEET_EXTENSION: AddOnDefinition = {
  ...
  yearlyDiscount: 21.6, // (9*12*0.8) = 86.40, Ersparnis: 21.60
  description:
    "Nur für Starter-Tarif. Pro zusätzlichem Fahrzeug oder Fahrer über die ersten 3 hinaus. Beliebig erweiterbar. Sofort aktiv. Monatlich kündbar.",
  applicableTo: ["starter"],
};

export const ALL_ADDONS: AddOnDefinition[] = [ADDON_FLEET_EXTENSION];
...
// ==================================================================================
export const FEATURE_TARIFF_MATRIX: Record<
  string,
  {
    name: string;
    route: string;
