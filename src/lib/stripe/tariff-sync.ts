...
export async function validateTariffSync(): Promise<boolean> {
  try {
    ...
    const allProductIds = ALL_TARIFFS.flatMap((t) => t.stripeProductIds as readonly string[]);

    const invalidCompanies = companies.filter(
      (c) => c.subscription_product_id && !allProductIds.includes(c.subscription_product_id)
    );
 ...
 */
export async function triggerStripeSyncEdgeFunction(
