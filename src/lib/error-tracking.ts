...
export async function logError(
  ...
  try {
    if (import.meta.env.DEV) {
      ...
      logError("Error (DEV)", error instanceof Error ? error : new Error(String(error)), {
        componentName,
        additionalInfo,
      });
      return;
...
export async function logWarning(
  ...
  try {
    ...
    const ignorePatterns = ["ResizeObserver loop limit exceeded", "Script error"];
    ...
    await supabase.from("error_logs").insert(warningLog);
  } catch (err) {
    const { logError } = await import("@/lib/logger");
