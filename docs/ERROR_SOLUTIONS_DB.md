...
5. ⚠️ `src/hooks/use-shifts.tsx` - DELETE bleibt (keine archived-Spalte in DB)
   - **TODO V18.4:** Migration für archived-System bei shifts-Tabelle

**Scanner behalten (kein Fix nötig):**
6. ✅ `src/lib/agent-debug-system.ts` - Scanner-Code (sucht nach .delete())
7. ✅ `src/lib/auto-optimization/system-scanner.ts` - Scanner-Code
