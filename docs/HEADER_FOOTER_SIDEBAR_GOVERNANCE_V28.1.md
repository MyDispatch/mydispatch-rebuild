...
```tsx
className =
  "h-11 px-8 font-semibold rounded-lg text-sm bg-slate-600 text-white border border-slate-600 shadow-sm hover:bg-slate-700 hover:shadow-md transition-all duration-300";
```
...
{
  company?.logo_url ? <img src={company.logo_url} alt="Logo" /> : <span>{company?.name}</span>;
}
```
