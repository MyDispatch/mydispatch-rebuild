...
// Verwendung
const { data } = await fetchWithRetry(() => supabase.from("customers").select("*"));
```
...
```typescript
// ✅ RICHTIG - Mit Timeout
const fetchWithTimeout = async (fn: () => Promise<any>, timeout = 10000): Promise<any> => {
  return Promise.race([
    fn(),
...
if (!validationError) {
  return new Response(JSON.stringify({ error: "Validation failed", details: validationError }), {
    status: 400,
    headers: corsHeaders,
  });
}
