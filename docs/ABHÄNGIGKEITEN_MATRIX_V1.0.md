...
const frontendBackendDeps = {
  ...
  // Aufträge
  "/auftraege": ["get-bookings", "create-booking", "update-booking", "delete-booking"],

  // Fahrer
  "/fahrer": ["get-drivers", "create-driver", "send-driver-invitation"],

  // Rechnungen
  "/rechnungen": ["get-invoices", "create-invoice", "export-invoice-pdf"],

  // AI Features
  "/ai-assistant": ["dashboard-ai-assistant", "ai-orchestrator", "kronos-executor"],
};
```
...
const externalAPIs = {
  ...
  lovableAI: {
    ...
    models: ["google/gemini-2.5-pro", "google/gemini-2.5-flash", "openai/gpt-5"],
  ...
  resend: {
    ...
    usedBy: ["send-booking-email", "send-driver-invitation", "send-password-reset"],
...
const criticalPaths = {
  ...
  auth: {
    ...
    dependencies: ["supabase.auth.signInWithPassword", "auth.users table", "RLS policies"],
...
const impactAnalysis = {
  ...
  "ALTER TABLE bookings": {
    affectedFunctions: ["get-bookings", "create-booking", "update-booking"],
  ...
  "kronos-executor body change": {
    affectedCallers: ["/pages/KronosOrchestrator.tsx", "useAutonomousMigration hook"],
    affectedFunctions: [],
  ...
  // API Endpoint Change
  "Stripe API v2 → v3": {
    affectedFunctions: ["create-checkout", "customer-portal", "stripe-webhook"],
...
const monitorDependencies = async () => {
  ...
  const critical = [results.authSystem, results.databaseConnections];
