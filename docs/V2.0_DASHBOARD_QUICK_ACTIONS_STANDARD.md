...
```
┌─────────────────────────────┐
│  Quick Actions Card         │ ← Primäre Aktionen
│  • Action 1                 │
│  • Action 2                 │
│  • Action 3                 │
├─────────────────────────────┤
│  Recent Activity Card       │ ← Letzte Aktivitäten
│  • Activity 1 (Zeit)        │
│  • Activity 2 (Zeit)        │
│  • Activity 3 (Zeit)        │
├─────────────────────────────┤
│  Context Widget Card        │ ← Kontext-spezifisches Widget
│  [SystemStatus | QuickStats │
│   Shortcuts | Events]       │
└─────────────────────────────┘
```
...
- Max-Height: `calc(100vh - 200px)` für Viewport-Optimierung
...
**Verwendung:** Aufträge, Kunden, Fahrer, Rechnungen
...
> = {
  master: {
    ...
    recentActivity: {
      items: [
        { icon: CheckCircle2, iconColor: 'text-green-600', titleKey: 'deploymentSuccessful', relativeTime: { value: 2, unit: 'hours' } },
        { icon: AlertTriangle, iconColor: 'text-amber-600', titleKey: 'highCpuUsage', relativeTime: { value: 5, unit: 'hours' } },
        { icon: CheckCircle2, iconColor: 'text-green-600', titleKey: 'backupCompleted', relativeTime: { value: 1, unit: 'days' } },
      ],
    },
    contextWidget: {
      type: 'systemStatus',
