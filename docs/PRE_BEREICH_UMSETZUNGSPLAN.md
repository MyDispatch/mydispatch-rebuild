...
- ✅ **WCAG 2.1 AA** (volle Accessibility)
- ✅ **Design System V28.1** (strikte Einhaltung)
- ✅ **Vollständige Dokumentation** (jeden Schritt dokumentiert)
- ✅ **Umfassende Tests** (Unit, E2E, Visual, a11y)
...
- `VORSCHRIFT_SEITENAUFBAU_HERO.md` - Hero-Vorgaben für Pre-Bereich
...
2. **Features Pages:**
   ...
   - `/features/automatisierung` - KI-gestützte Automatisierung
   - `/features/rechnungsstellung` - Automatische Rechnungen
   - `/features/api` - API-Integration & Dokumentation

3. **Branchen Pages:**
   - `/branchen/taxi` - Taxi-spezifische Lösungen
   - `/branchen/mietwagen` - Mietwagen-Optimierung
   - `/branchen/limousinen` - Premium Limousinen-Service

4. **Service Pages:**
   - `/demo` - Demo anfragen (Form + Grafik)
   - `/contact` ✅ EXISTIERT - Kontaktformular

5. **Info Pages:**
   - `/pricing` ✅ EXISTIERT - Tarife (OHNE Grafik, nur Pricing Cards im Hero)
   - `/faq` ✅ EXISTIERT - FAQ mit Suchfunktion
   - `/docs` ✅ EXISTIERT - Dokumentation

6. **Legal Pages (OHNE Grafik, Pricing-Style Hero):**
   - `/impressum` ✅ EXISTIERT
   - `/datenschutz` ✅ EXISTIERT
   - `/agb` ✅ EXISTIERT
   - `/terms` ✅ EXISTIERT
...
### 2.1 BENÖTIGTE COMPONENTS (61 TOTAL)
...
**Complex (9):** CookieConsent ✅, CookieSettings, Search, NewsletterSignup, ContactForm, ComparisonTable, FAQ ✅, Carousel, CTASection
...
4. ✅ Accessibility implementieren (WCAG 2.1 AA)
5. ✅ Tests schreiben (Unit, Visual, a11y)
6. ✅ Dokumentation updaten (Registry, Usage Guide)
7. ✅ Self-Review Checklist durchgehen
...
## 🔍 PHASE 4: FEHLERSUCHE & QUALITY GATES
...
- Text/Microcopy (Ton, Gendersensibilität, Konsistenz)
...
- ✅ Feedback: Toast, CookieConsent
- ✅ Forms: Contact Form (weitere prüfen)
- ✅ Complex: FAQ, Accordion
...
- 🟡 Testing (Unit Tests vorhanden, E2E teilweise)
- 🟡 Performance (Home optimiert, andere prüfen)
- 🟡 Documentation (teilweise, muss erweitert werden)
...
2. `PROJECT_MEMORY.md` erstellen:
   ...
   - Häufige Halluzinationen notieren
...
- ❌ NIEMALS ohne Accessibility (WCAG 2.1 AA)
- ❌ NIEMALS ohne Tests (Unit + E2E minimum)
- ❌ NIEMALS ohne Dokumentation (Registry + Changelog)
- ✅ IMMER 3-Phasen-Workflow befolgen
- ✅ IMMER AAA Triple Check vor Commit
- ✅ IMMER Self-Review Loop durchlaufen
...
**Version:** 1.0.0  
**Erstellt:** 2025-10-28  
**Nächstes Update:** Bei Phase 1 Start
