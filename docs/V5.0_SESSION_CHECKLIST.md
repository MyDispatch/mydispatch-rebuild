# 🔄 V5.0 SESSION INITIALIZATION CHECKLIST

**Version:** 5.0  
**Zweck:** Mandatory Workflow für jeden neuen Chat-Start  
**Status:** ✅ PRODUCTION-READY

---

## 📋 MANDATORY SESSION-START-WORKFLOW

Bei **JEDEM** neuen Chat-Start (Session Initialization) MÜSSEN folgende Schritte ausgeführt werden:

### **PHASE 0: Knowledge-Base-Sync (CRITICAL)**

```typescript
// VERPFLICHTEND bei jedem Session-Start!
await supabase.functions.invoke('sync-docs-to-knowledge-base', {
  body: { force_refresh: false }
});
```

**Zweck:** Synchronisiert alle wichtigen Dokumentationen in die `knowledge_base` Tabelle.

**Erfolgs-Kriterien:**
- ✅ Alle Core-Docs in `knowledge_base` vorhanden
- ✅ `confidence_score` bei 1.0 für kritische Entries
- ✅ Keine veralteten Einträge (>90 Tage)

---

### **PHASE 1: Core Documentation Lesen**

```typescript
const coreDocs = [
  'docs/PROJECT_MEMORY.md',
  'docs/COMPONENT_REGISTRY.md',
  'docs/LESSONS_LEARNED.md',
  'docs/AVOIDABLE_ERRORS.md',
  'docs/V5.0_MIGRATION_COMPLETE.md'
];
```

**Zweck:** Lade alle wichtigen Projekt-Dokumentationen in den Kontext.

---

### **PHASE 2: Knowledge-Base Abfragen**

```typescript
// Lade neueste AI Learning Patterns
const { data: recentLearnings } = await supabase
  .from('ai_learning_patterns')
  .select('*')
  .order('learned_at', { ascending: false })
  .limit(10);

console.log('✅ Recent Learnings loaded:', recentLearnings.length);
```

**Zweck:** Lade die neuesten AI Learning Patterns aus der letzten Session.

---

### **PHASE 3: Known Issues Check (CRITICAL)**

```typescript
// Lade ALLE ungelösten Critical Issues
const { data: criticalIssues } = await supabase
  .from('known_issues')
  .select('*')
  .eq('severity', 'critical')
  .eq('resolved', false);

if (criticalIssues.length > 0) {
  console.warn('⚠️ CRITICAL ISSUES:', criticalIssues);
}
```

**Zweck:** Identifiziere alle kritischen, ungelösten Probleme.

**Warnung:** Wenn `criticalIssues.length > 0` → Warnung an User ausgeben!

---

### **PHASE 4: Component Registry Laden**

```typescript
// Lade ALLE existierenden Components
const { data: allComponents } = await supabase
  .from('component_registry')
  .select('component_name, file_path, verification_status')
  .eq('verification_status', 'active');

console.log('✅ Known Components:', allComponents.length);
```

**Zweck:** Verhindere Recreation von existierenden Components.

**Erfolgs-Kriterien:**
- ✅ Mindestens 20 Components registriert
- ✅ V28Button, V28Card, V28IconBox vorhanden

---

### **PHASE 5: Design System V28.1 Validierung**

```typescript
// Prüfe ob V28.1 Design System korrekt ist
const v28Components = await supabase
  .from('component_registry')
  .select('*')
  .or('component_name.eq.V28Button,component_name.eq.V28Card,component_name.eq.V28IconBox');

if (v28Components.data?.length !== 3) {
  console.error('❌ V28 Design System incomplete!');
}
```

**Zweck:** Sicherstellen, dass V28.1 Design System vollständig ist.

**Erfolgs-Kriterien:**
- ✅ V28Button existiert
- ✅ V28Card existiert
- ✅ V28IconBox existiert

---

### **PHASE 6: Self-Monitoring Check**

```typescript
// Prüfe letzten Self-Report
const { data: lastReport } = await supabase
  .from('ai_self_reports')
  .select('*')
  .order('report_date', { ascending: false })
  .limit(1);

if (lastReport && lastReport.length > 0) {
  const daysSinceLastReport = Math.floor(
    (Date.now() - new Date(lastReport[0].report_date).getTime()) / (1000 * 60 * 60 * 24)
  );
  
  if (daysSinceLastReport > 7) {
    console.warn('⚠️ Self-Report veraltet! Letzter Report vor', daysSinceLastReport, 'Tagen');
  }
}
```

**Zweck:** Prüfe ob wöchentlicher Self-Report aktuell ist.

**Erfolgs-Kriterien:**
- ✅ Letzter Report < 7 Tage alt
- ✅ `metrics.knowledge_check_compliance >= 0.9`
- ✅ `metrics.hallucination_rate <= 0.01`

---

## 📊 SESSION INITIALIZATION SUMMARY

```typescript
console.log('✅ Session initialized mit:');
console.log('  - Learnings:', recentLearnings.data.length);
console.log('  - Critical Issues:', criticalIssues.data.length);
console.log('  - Known Components:', allComponents.data.length);
console.log('  - V28 Design System:', v28Components.data?.length === 3 ? '✅' : '❌');
console.log('  - Last Self-Report:', daysSinceLastReport, 'days ago');
```

---

## 🚨 FAIL-SAFES

### **Wenn Knowledge-Base-Sync fehlschlägt:**

```typescript
if (syncResult.errors > 0) {
  console.error('❌ Knowledge-Base-Sync failed!');
  // FALLBACK: Lese direkt aus docs/ Ordner
}
```

### **Wenn Critical Issues gefunden:**

```typescript
if (criticalIssues.length > 0) {
  console.warn('⚠️ WARNUNG: Kritische Issues gefunden!');
  // User benachrichtigen
}
```

### **Wenn V28 Design System incomplete:**

```typescript
if (v28Components.data?.length !== 3) {
  console.error('❌ V28 Design System NICHT vollständig!');
  // Warnung an User + Empfehlung zur Neuregistrierung
}
```

---

## 🎯 ERFOLGS-KRITERIEN FÜR SESSION-START

| Kriterium | Ziel | Status |
|-----------|------|--------|
| Knowledge-Base-Sync | Success = true | ✅ |
| Core Docs geladen | Alle 5 Docs | ✅ |
| Recent Learnings | >= 5 Entries | ✅ |
| Critical Issues | 0 ungelöste | ✅ |
| Known Components | >= 20 Components | ✅ |
| V28 Design System | 3/3 Components | ✅ |
| Last Self-Report | < 7 Tage alt | ✅ |

---

## 📝 INTEGRATION IN NEXIFY WORKFLOW

**In `NEXIFY_WORKFLOW_PROMPT_V19.0.0.md`:**

```
🔄 NEXIFY WORKFLOW-START V19.0.0

PHASE 0: SESSION INITIALIZATION (NEU!)
- Knowledge-Base-Sync ausführen
- Core Docs laden
- Known Issues Check
- Component Registry laden
- V28.1 Design System validieren

PHASE 1: SELBSTREFLEXION & CODE-AUDIT
...
```

---

## 🔧 AUTOMATISIERUNG (OPTIONAL)

**GitHub Action für wöchentlichen Self-Review:**

```yaml
# .github/workflows/weekly-self-review.yml
name: Weekly AI Self-Review
on:
  schedule:
    - cron: '0 0 * * 1' # Jeden Montag um 00:00 UTC
  workflow_dispatch:

jobs:
  self-review:
    runs-on: ubuntu-latest
    steps:
      - name: Trigger Self-Review
        run: |
          curl -X POST https://vsbqyqhzxmwezlhzdmfd.supabase.co/functions/v1/weekly-self-review \
            -H "Authorization: Bearer ${{ secrets.SUPABASE_SERVICE_ROLE_KEY }}" \
            -H "Content-Type: application/json" \
            -d '{"review_period": "last_7_days"}'
```

---

**Version:** 5.0  
**Datum:** 2025-01-30  
**Status:** ✅ PRODUCTION-READY

**MISSION:** Zero-Hallucination durch perfekte Session-Initialization!
