...
import { CheckCircle2, XCircle, Loader2, Database, FileCode, AlertCircle, Code2, CheckSquare, Activity } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50 p-8">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <Card className={
          apiHealth?.overall_status === 'healthy' ? 'border-green-500' : 
          apiHealth?.overall_status === 'critical' ? 'border-red-500' : 
          'border-yellow-500'
        }>
          <CardHeader>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                    apiHealth.overall_status === 'healthy' ? 'bg-slate-100 text-slate-800' :
                    apiHealth.overall_status === 'critical' ? 'bg-slate-200 text-slate-900' :
                ...
                <div className="space-y-2">
                  {apiHealth.apis.map((api: any) => (
                    <div key={api.api_name} className="flex items-center justify-between p-2 bg-slate-50 rounded">
                      <div>
                        <span className="font-medium text-sm">{api.api_name}</span>
                        {api.critical && <span className="ml-2 text-xs text-orange-600">(Critical)</span>}
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-xs text-slate-600">{api.response_time_ms}ms</span>
                        <span className={`px-2 py-1 rounded text-xs font-medium ${
                          api.status === 'healthy' ? 'bg-slate-100 text-slate-800' :
                          api.status === 'down' ? 'bg-slate-200 text-slate-900' :
                          'bg-slate-100 text-slate-700'
                        }`}>
                          {api.status}
                        </span>
        ...
        <Card className="border-slate-200 bg-slate-50">
          ...
          <CardContent className="space-y-2 text-sm text-slate-700">
            <p><strong>Phase 0:</strong> Schema-Fix (✅ Completed - pattern_name + API tables added)</p>
            <p><strong>Phase 1:</strong> 112 MD-Files → Knowledge-Base mit Tavily-Validation (Expected: +150-200 entries)</p>
            <p><strong>Phase 2:</strong> Component-Scan → Component-Registry (Expected: 60+ components)</p>
            <p><strong>Phase 3:</strong> Known-Issues → Solutions via Tavily-Search (Expected: 22 issues + best practices)</p>
            <p><strong>Phase 4:</strong> Code-Snippets → Wiederverwendbare Patterns mit Tavily-Validator (Expected: +15-20 snippets)</p>
            <p><strong>Phase 5:</strong> Final-Validation → Consistency-Check + API-Health (Expected: 342+ total entries)</p>
            <div className="pt-4 mt-4 border-t border-slate-300">
              <p className="text-slate-600"><strong>Total Duration:</strong> ~90 Minuten (mit Tavily-Integration)</p>
              <p className="text-slate-600"><strong>Expected Result:</strong> Zero-Hallucination Knowledge-System mit Live-Best-Practices</p>
              <p className="text-slate-700 font-medium"><strong>NEW:</strong> Alle Einträge werden gegen Tavily Official Docs validiert! 🎯</p>
            </div>
          </CardContent>
        </Card>
      </div>
