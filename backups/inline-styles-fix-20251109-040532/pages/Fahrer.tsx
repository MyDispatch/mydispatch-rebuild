...
import { Users, Download, Mail, UserPlus, Edit, Car, Eye, Plus, Activity, Wrench, Phone, MapPin } from 'lucide-react';
import { StatusIndicator, getDriverStatusType, getVehicleStatusType } from '@/components/shared/StatusIndicator';
...
const Fahrer = () => {
  ...
  const activeDrivers = useMemo(() => filteredDrivers.filter(d => !d.archived), [filteredDrivers]);
  const availableDrivers = useMemo(() => activeDrivers.filter(d => d.shift_status === 'available'), [activeDrivers]);
  const busyDrivers = useMemo(() => activeDrivers.filter(d => d.shift_status === 'busy'), [activeDrivers]);
  
  const activeVehicles = useMemo(() => filteredVehicles.filter(v => !v.archived), [filteredVehicles]);
  const availableVehicles = useMemo(() => activeVehicles.filter(v => v.status === 'available'), [activeVehicles]);
  const maintenanceVehicles = useMemo(() => activeVehicles.filter(v => v.status === 'wartung'), [activeVehicles]);
  ...
  ] as [any, any, any], [activeDrivers, availableDrivers, busyDrivers]);
  ...
  ] as [any, any, any], [activeVehicles, availableVehicles, maintenanceVehicles]);
      ...
      <>
        ...
        >
          <StandardPageLayout
            ...
            searchPlaceholder={currentTab === 'fahrer' ? "Fahrer durchsuchen..." : "Fahrzeuge durchsuchen..."}
          >
            ...
            {currentTab === 'fahrer' ? (
              ...
              <MobileFahrzeuge
                vehicles={filteredVehicles.map(v => ({
                  ...
                  brand: v.brand || '',
                  model: v.model || '',
                  status: (v.status === 'wartung' ? 'maintenance' : v.status === 'defekt' ? 'offline' : v.status) as 'available' | 'maintenance' | 'offline',
  ...
  const renderDriverTable = () => {
    const isAllSelected = activeDrivers.length > 0 && activeDrivers.every(d => bulkSelectionDrivers.selectedIds.includes(d.id));
                ...
                <TableRow>
                  <TableCell colSpan={6} className="p-0">
                    <EmptyState
                      ...
                      description={searchTerm ? 'Versuchen Sie eine andere Suchanfrage' : 'Fügen Sie Ihren ersten Fahrer hinzu'}
  ...
  const renderVehicleTable = () => {
    const isAllSelected = activeVehicles.length > 0 && activeVehicles.every(v => bulkSelectionVehicles.selectedIds.includes(v.id));
    ...
    return (
      <>
                ...
                <TableRow>
                  <TableCell colSpan={6} className="p-0">
                    <EmptyState
                      ...
                      title={searchTerm ? 'Keine Fahrzeuge gefunden' : 'Noch keine Fahrzeuge vorhanden'}
                      description={searchTerm ? 'Versuchen Sie eine andere Suchanfrage' : 'Fügen Sie Ihr erstes Fahrzeug hinzu'}
                      isSearchResult={!!searchTerm}
                    />
                  </TableCell>
                </TableRow>
        ...
        <BulkActionBar
          ...
          actions={[
            { label: 'Status ändern', icon: Wrench, onClick: () => toast({ title: 'Bald verfügbar' }) },
  ...
  return (
    <>
      <StandardPageLayout
        ...
        cardIcon={currentTab === 'fahrer' ? <Users className="h-5 w-5" /> : <Car className="h-5 w-5" />}
      >
              ...
              <>
                ...
                {vehicles.find(v => v.assigned_driver_id === selectedDriver.id) && (
                  <RelatedEntityCard
                    ...
                    value={vehicles.find(v => v.assigned_driver_id === selectedDriver.id)?.license_plate || ''}
                    ...
                    onClick={() => {
                      const vehicle = vehicles.find(v => v.assigned_driver_id === selectedDriver.id);
                      if (vehicle) {
          ...
          >
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <V28Button
                  variant="secondary"
                  onClick={() => navigate('/schichtzettel', { state: { driverId: selectedDriver.id } })}
                >
              ...
              <>
                ...
                {drivers.find(d => d.id === selectedVehicle.assigned_driver_id) && (
                  <RelatedEntityCard
                    ...
                    meta={drivers.find(d => d.id === selectedVehicle.assigned_driver_id)?.email || drivers.find(d => d.id === selectedVehicle.assigned_driver_id)?.phone || ''}
                    onClick={() => {
                      const driver = drivers.find(d => d.id === selectedVehicle.assigned_driver_id);
                      if (driver) {
                        setDetailDialogOpen(false);
                        setSelectedDriver(driver);
                        setDetailDialogOpen(true);
                    ...
                    actions={[
                      ...(drivers.find(d => d.id === selectedVehicle.assigned_driver_id)?.phone ? [getStandardActions.phone(drivers.find(d => d.id === selectedVehicle.assigned_driver_id)!.phone!)] : []),
                      ...(drivers.find(d => d.id === selectedVehicle.assigned_driver_id)?.email ? [getStandardActions.email(drivers.find(d => d.id === selectedVehicle.assigned_driver_id)!.email!)] : []),
                    ]}
                  />
                )}
              </>
          ...
          >
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                {selectedVehicle.tuev_expiry_date && (
                  <div>
                    ...
                    <p className="font-medium">{format(new Date(selectedVehicle.tuev_expiry_date), 'dd.MM.yyyy', { locale: de })}</p>
                  </div>
                )}
              ...
              <div className="flex gap-2">
                ...
                <V28Button
                  variant="secondary"
                  onClick={() => toast({ title: 'Wartungsplan öffnen', description: 'Bald verfügbar' })}
                >
                ...
                <V28Button
                  variant="secondary"
                  onClick={() => navigate('/auftraege', { state: { vehicleId: selectedVehicle.id } })}
                >
            ...
            {currentTab === 'fahrer' ? (
              <div className="space-y-2">
                ...
                <div className="p-3 bg-status-success/10 rounded-lg border border-status-success/20">
                  ...
                  <p className="text-2xl font-bold text-status-success">{availableDrivers.length}</p>
                  <p className="text-xs text-green-500 mt-1">
                    {Math.round((availableDrivers.length / (activeDrivers.length || 1)) * 100)}% der Flotte
                  </p>
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="p-3 bg-status-success/10 rounded-lg border border-status-success/20">
                  ...
                  <p className="text-2xl font-bold text-status-success">{availableVehicles.length}</p>
                  <p className="text-xs text-green-500 mt-1">
                    {Math.round((availableVehicles.length / (activeVehicles.length || 1)) * 100)}% einsatzbereit
                  </p>
                </div>
