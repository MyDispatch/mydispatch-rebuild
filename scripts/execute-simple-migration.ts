...
const SIMPLE_COMPONENTS = ["Button", "Input", "Card", "Badge", "Select"];
...
function usesOnlySimpleComponents(content: string): boolean {
  ...
  for (const match of matches) {
    ...
    if (
      !SIMPLE_COMPONENTS.some(
        (simple) => componentName === simple.toLowerCase() || componentName === simple
      )
    ) {
  ...
  try {
    ...
    for (const [oldComp, config] of Object.entries(COMPONENT_MAP)) {
      ...
      if (oldImportPattern.test(content)) {
        ...
        content = content.replace(oldImportPattern, (match) => {
...
async function main() {
  ...
  // Write report
  writeFileSync("logs/migration-report-phase2a.json", JSON.stringify(report, null, 2), "utf-8");
  ...
  console.log(
    `   5. Commit: git add . && git commit -m "feat: Phase 2A - Simple Component Migration"`
  );
  console.log("");
