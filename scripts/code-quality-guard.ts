      ...
      pricingPatterns.forEach(pattern => {
        if (pattern.test(line)) {
          issues.push({
            ...
            message: 'Hardcoded Pricing verboten. Importiere: import { PRICING_TIERS } from "@/data/pricing-tiers"',
            severity: 'error',
            autofix: 'import { PRICING_TIERS } from "@/data/pricing-tiers";\n// Verwende: PRICING_TIERS.starter.price'
          });
        }
      });
    });
      ...
      colorPatterns.forEach(pattern => {
        if (pattern.test(line)) {
          issues.push({
            ...
            message: 'Direkte Farben verboten. Verwende Design-System-Tokens (text-foreground, bg-primary, etc.)',
            severity: 'error',
