...
async function activateSystem() {
  try {
    ...
    const { data: configData, error: configError } = await supabase
      ...
      .eq('id', (await supabase.from('autonomous_system_config').select('id').limit(1).single()).data?.id);
    ...
    const { data: pollResult, error: pollError } = await supabase.functions.invoke('ai-agent-poll', {
      body: {}
    });
