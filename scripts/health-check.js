...
async function completeSystemHealthCheck() {
  ...
  if (supabaseUrl && supabaseKey) {
    try {
      ...
      if (error) {
        health.critical.push({ name: 'Supabase', status: 'warning', details: `Verbindung: ${error.message}` });
      ...
      health.critical.push({ name: 'Supabase', status: 'error', details: `Fehler: ${error.message}` });
  ...
  if (existsSync(packageJsonPath)) {
    try {
      ...
      if (missingScripts.length === 0) {
        ...
        health.important.push({ name: 'Scripts', status: 'warning', details: `${missingScripts.length} fehlen` });
        console.log(`   ⚠️  Scripts: ${missingScripts.length} fehlen (${missingScripts.join(', ')})`);
      }
    } catch (error) {
