...
class AutoHealer {
  ...
  async healFile(file: string, rules: HealingRule[]): Promise<HealingResult[]> {
    ...
    for (const rule of rules) {
      try {
        ...
        const fixedContent = typeof rule.replacement === 'function'
  ...
  async healAll(category?: HealingRule['category']) {
    ...
    console.log(`📊 Analyzing ${files.length} files with ${rulesToApply.length} healing rules...\n`);
    ...
    // Group by category
    const byCategory = rulesToApply.reduce((acc, rule) => {
      acc[rule.category] = (acc[rule.category] || 0) + 
        this.healingHistory.filter(h => h.rule === rule.id && h.success).reduce((s, h) => s + h.fixes, 0);
      return acc;
