...
const WORKFLOWS = {
  checkWiki: {
    ...
    command: () =>
      existsSync(WIKI_PATH)
        ? Promise.resolve(true)
        : Promise.reject(new Error("Wiki nicht gefunden")),
    critical: true,
  },
  checkCredentials: {
    name: "Credentials Prüfung",
    command: () =>
      existsSync(ENV_LOCAL_PATH)
        ? Promise.resolve(true)
        : Promise.reject(new Error("Credentials nicht gefunden")),
    critical: true,
  },
  validateTypeScript: {
    name: "TypeScript Check",
    command: () =>
      execAsync("npm run type-check", { cwd: PROJECT_ROOT, maxBuffer: 10 * 1024 * 1024 }),
    critical: false,
  },
  validateAll: {
    name: "Vollständige Validierung",
    command: () =>
      execAsync("npm run validate:all", { cwd: PROJECT_ROOT, maxBuffer: 10 * 1024 * 1024 }),
    critical: false,
  },
};
