...
async function validateDeployments() {
  ...
  for (const tableName of expectedTables) {
    try {
      ...
      if (error && (error.code === "42P01" || error.message.includes("Could not find the table"))) {
        ...
        results.tables.checks.push({
          table: tableName,
          status: "⚠️ Noch nicht erstellt (Migrations erforderlich)",
        });
      ...
      results.tables.checks.push({
        table: tableName,
        status: `❌ Exception: ${err.message || err}`,
      });
    ...
    if (error) {
      ...
      if (
        ...
        results.rls.checks.push({
          status: "⚠️ RPC Funktion noch nicht verfügbar (Migrations erforderlich)",
        });
      } else {
  ...
  const missingTables = results.tables.checks.filter((c) =>
    c.status.includes("Noch nicht erstellt")
  ).length;
  ...
  if (missingTables > 0) {
    console.log(
      `ℹ️  ${missingTables} Tabellen noch nicht erstellt (Migrations erforderlich - normal)`
    );
  }

  const rlsStatus =
    results.rls.passed > 0
      ? "✅ OK"
      : results.rls.checks.some((c) => c.status.includes("noch nicht verfügbar"))
        ? "⚠️ Funktion noch nicht verfügbar"
        : "❌ Probleme";
