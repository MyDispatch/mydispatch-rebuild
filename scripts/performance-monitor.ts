...
class PerformanceMonitor {
  ...
  private async runLighthouse(): Promise<PerformanceMetrics["lighthouse"] | undefined> {
    ...
    try {
      ...
      const output = execSync("npx lighthouse http://localhost:5173 --output json --quiet", {
        encoding: "utf-8",
      });
  ...
  private checkBudget(metrics: PerformanceMetrics) {
    ...
    if (bundleSizeKB > this.budget.maxBundleSize) {
      ...
      console.log(
        `✅ Bundle size within budget (${bundleSizeKB.toFixed(2)} KB / ${this.budget.maxBundleSize} KB)`
      );
    }
    ...
    if (buildTimeSec > this.budget.maxBuildTime) {
      ...
      console.log(
        `✅ Build time within budget (${buildTimeSec.toFixed(2)}s / ${this.budget.maxBuildTime}s)`
      );
    }
    ...
    if (metrics.lighthouse) {
      ...
      if (minScore < this.budget.minLighthouseScore) {
        ...
        console.log(
          `✅ Lighthouse scores above budget (min: ${minScore} / ${this.budget.minLighthouseScore})`
        );
      }
    }
  ...
  private showTrend() {
    ...
    console.log(
      `${bundleIcon} Bundle Size: ${bundleTrend > 0 ? "+" : ""}${bundleTrend.toFixed(2)} KB`
    );
    ...
    if (lighthouseScores.length >= 2) {
      ...
      console.log(
        `${lighthouseIcon} Lighthouse: ${lighthouseTrend > 0 ? "+" : ""}${lighthouseTrend}`
      );
    }
  }
}
