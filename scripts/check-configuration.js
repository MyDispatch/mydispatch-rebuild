...
const SETTINGS_PATH = join(process.env.APPDATA || "", "Cursor", "User", "settings.json");
    ...
    try {
      ...
      if (autoApproveCount >= 10) {
        checks.critical.push({
          name: "Auto-Approval",
          status: "ok",
          details: `${autoApproveCount}/${autoApproveKeys.length} aktiviert`,
        });
        ...
        checks.critical.push({
          name: "Auto-Approval",
          status: "warning",
          details: `Nur ${autoApproveCount}/${autoApproveKeys.length} aktiviert`,
        });
        checks.issues.push(
          `Auto-Approval: Nur ${autoApproveCount}/${autoApproveKeys.length} aktiviert`
        );
        console.log(
          `   ⚠️  Auto-Approval: Nur ${autoApproveCount}/${autoApproveKeys.length} aktiviert`
        );
      }
      ...
      if (skipConfirmationCount >= 8) {
        checks.critical.push({
          ...
          status: "ok",
          details: `${skipConfirmationCount}/${skipConfirmationKeys.length} aktiviert`,
        });
        console.log(
          `   ✅ Skip Confirmation: ${skipConfirmationCount}/${skipConfirmationKeys.length} aktiviert`
        );
      } else {
        checks.critical.push({
          name: "Skip Confirmation",
          status: "warning",
          details: `Nur ${skipConfirmationCount}/${skipConfirmationKeys.length} aktiviert`,
        });
        checks.issues.push(
          `Skip Confirmation: Nur ${skipConfirmationCount}/${skipConfirmationKeys.length} aktiviert`
        );
        console.log(
          `   ⚠️  Skip Confirmation: Nur ${skipConfirmationCount}/${skipConfirmationKeys.length} aktiviert`
        );
      }
      ...
      if (requireApprovalCount >= 8) {
        checks.critical.push({
          ...
          status: "ok",
          details: `${requireApprovalCount}/${requireApprovalKeys.length} deaktiviert`,
        });
        console.log(
          `   ✅ Require Approval: ${requireApprovalCount}/${requireApprovalKeys.length} deaktiviert`
        );
      } else {
        checks.critical.push({
          name: "Require Approval",
          status: "warning",
          details: `Nur ${requireApprovalCount}/${requireApprovalKeys.length} deaktiviert`,
        });
        checks.issues.push(
          `Require Approval: Nur ${requireApprovalCount}/${requireApprovalKeys.length} deaktiviert`
        );
        console.log(
          `   ⚠️  Require Approval: Nur ${requireApprovalCount}/${requireApprovalKeys.length} deaktiviert`
        );
      }
    } catch (error) {
      checks.critical.push({
        name: "Settings.json",
        status: "error",
        details: `Parse-Fehler: ${error.message}`,
      });
    ...
    try {
      ...
      if (gitConfirmCount >= 3) {
        checks.important.push({
          ...
          status: "ok",
          details: `${gitConfirmCount}/${gitConfirmKeys.length} deaktiviert`,
        });
        console.log(
          `   ✅ Git Confirmations: ${gitConfirmCount}/${gitConfirmKeys.length} deaktiviert`
        );
      } else {
        checks.important.push({
          name: "Git Confirmations",
          status: "warning",
          details: `Nur ${gitConfirmCount}/${gitConfirmKeys.length} deaktiviert`,
        });
        console.log(
          `   ⚠️  Git Confirmations: Nur ${gitConfirmCount}/${gitConfirmKeys.length} deaktiviert`
        );
      }
      ...
      if (fileConfirmCount >= 2) {
        checks.important.push({
          name: "File Confirmations",
          status: "ok",
          details: `${fileConfirmCount}/${fileConfirmKeys.length} deaktiviert`,
        });
        console.log(
          `   ✅ File Confirmations: ${fileConfirmCount}/${fileConfirmKeys.length} deaktiviert`
        );
      } else {
        checks.important.push({
          name: "File Confirmations",
          status: "warning",
          details: `Nur ${fileConfirmCount}/${fileConfirmKeys.length} deaktiviert`,
        });
        console.log(
          `   ⚠️  File Confirmations: Nur ${fileConfirmCount}/${fileConfirmKeys.length} deaktiviert`
        );
      }
      ...
      if (terminalConfirmCount >= 1) {
        checks.important.push({
          name: "Terminal Confirmations",
          status: "ok",
          details: `${terminalConfirmCount}/${terminalConfirmKeys.length} deaktiviert`,
        });
        console.log(
          `   ✅ Terminal Confirmations: ${terminalConfirmCount}/${terminalConfirmKeys.length} deaktiviert`
        );
      } else {
        checks.important.push({
          name: "Terminal Confirmations",
          status: "warning",
          details: `Nur ${terminalConfirmCount}/${terminalConfirmKeys.length} deaktiviert`,
        });
        console.log(
          `   ⚠️  Terminal Confirmations: Nur ${terminalConfirmCount}/${terminalConfirmKeys.length} deaktiviert`
        );
      }
      ...
      if (workbenchConfirmCount >= 1) {
        checks.important.push({
          name: "Workbench Confirmations",
          status: "ok",
          details: `${workbenchConfirmCount}/${workbenchConfirmKeys.length} deaktiviert`,
        });
        console.log(
          `   ✅ Workbench Confirmations: ${workbenchConfirmCount}/${workbenchConfirmKeys.length} deaktiviert`
        );
      } else {
        checks.important.push({
          name: "Workbench Confirmations",
          status: "warning",
          details: `Nur ${workbenchConfirmCount}/${workbenchConfirmKeys.length} deaktiviert`,
        });
        console.log(
          `   ⚠️  Workbench Confirmations: Nur ${workbenchConfirmCount}/${workbenchConfirmKeys.length} deaktiviert`
        );
      }
    } catch (error) {
