...
function validateFileStructure(filePath: string): ValidationResult {
  ...
  const desktopHasPageHeaderWithKPIs = content.includes('<PageHeaderWithKPIs') &&
  ...
  const hasSidebar = /!isMobile &&[\s\S]*?<aside[\s\S]*?fixed right-0 top-16 bottom-0/.test(content);
...
function printResults(results: ValidationResult[]) {
  ...
  for (const result of results) {
    ...
    if (!result.passed) {
      ...
      if (result.viewportChecks) {
        ...
        console.log(`    Mobile: ${result.viewportChecks.mobile.passed ? '✅' : '❌'} ${result.viewportChecks.mobile.hasDirectStatCards ? 'Direct StatCards' : 'Missing direct StatCards'}`);
        console.log(`    Desktop: ${result.viewportChecks.desktop.passed ? '✅' : '❌'} ${result.viewportChecks.desktop.hasPageHeaderWithKPIs ? 'PageHeaderWithKPIs' : 'Missing PageHeaderWithKPIs'}`);
        console.log(`    Sidebar: ${result.viewportChecks.sidebar.passed ? '✅' : '❌'} ${result.viewportChecks.sidebar.hasRightSidebar ? 'Right Sidebar (320px)' : 'Missing right sidebar'}`);
      }
    } else {
      passedFiles++;
    }
  ...
  if (totalViolations > 0) {
    console.log('💡 Fix violations by ensuring all pages follow /rechnungen Golden Template structure');
    console.log('   Required components: StandardPageLayout, UniversalExportBar, StatCard, BulkActionBar');
    console.log('   Required pattern: Mobile/Desktop separation, V28Button usage, Bulk selection\n');
  }
}
