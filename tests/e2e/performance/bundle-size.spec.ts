...
test.describe('Performance Budget - Bundle Size', () => {
  ...
  test('should use code splitting for routes', async ({ page }) => {
    ...
    console.log(`JS files loaded: Home=${homeJsCount}, Dashboard=${dashboardJsCount}, Aufträge=${auftraegeJsCount}`);
  ...
  test('should compress assets (gzip/brotli)', async ({ page }) => {
    ...
    const compressionRatio = compressedResources.length / (compressedResources.length + uncompressedResources.length);
