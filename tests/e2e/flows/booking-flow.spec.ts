...
test.describe("Complete Booking Flow", () => {
  ...
  test("User can create a new booking from dashboard", async ({ page }) => {
    ...
    await page.fill(
      'input[type="datetime-local"], input[name="pickup_time"]',
      `${dateString}T14:00`
    );
  ...
  test("User can assign driver to booking", async ({ page }) => {
    ...
    if ((await firstRow.count()) > 0) {
      ...
      const assignButton = firstRow.locator(
        'button:has-text("Zuweisen"), button[aria-label*="assign" i]'
      );
      ...
      if ((await statusOption.count()) > 0) {
        ...
        if ((await statusSelect.count()) > 0) {
          ...
          const completedStatus = page.locator(
            '[role="option"]:has-text("Abgeschlossen"), [role="option"]:has-text("Completed")'
          );
  ...
  test("User can filter bookings by status", async ({ page }) => {
    ...
    if ((await statusFilter.count()) > 0) {
      ...
      const pendingOption = page.locator(
        '[role="option"]:has-text("Ausstehend"), [role="option"]:has-text("Pending")'
      );
