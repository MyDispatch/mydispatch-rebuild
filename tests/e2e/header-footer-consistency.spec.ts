...
test.describe('Spacing Consistency', () => {
  test('Desktop Header has correct padding', async ({ page }) => {
    ...
    // px-8 = 32px (2rem)
    const paddingLeft = await headerDiv.evaluate(el => 
      window.getComputedStyle(el).paddingLeft
    );
    const paddingRight = await headerDiv.evaluate(el => 
  ...
  test('Desktop Footer has correct padding', async ({ page }) => {
    ...
    const paddingLeft = await footerDiv.evaluate(el => 
      window.getComputedStyle(el).paddingLeft
    );
    const paddingRight = await footerDiv.evaluate(el => 
      window.getComputedStyle(el).paddingRight
    );
...
test.describe('Transition Synchronization', () => {
  ...
  test('Header and Footer adjust synchronously with Sidebar expansion', async ({ page }) => {
    ...
    const initialHeaderLeft = await header.evaluate(el => 
    ...
    const expandedHeaderLeft = await header.evaluate(el => 
      window.getComputedStyle(el).left
    );
    const expandedFooterLeft = await footer.evaluate(el => 
      window.getComputedStyle(el).left
    );
...
test.describe('Button Styling Consistency', () => {
  test('All primary buttons have consistent styling', async ({ page }) => {
    ...
    const height = await primaryButton.evaluate(el => 
      window.getComputedStyle(el).height
    );
    const paddingLeft = await primaryButton.evaluate(el => 
      window.getComputedStyle(el).paddingLeft
    );
    const paddingRight = await primaryButton.evaluate(el => 
      window.getComputedStyle(el).paddingRight
    );
...
test.describe('Logo Component Unification', () => {
  test('Mobile Header uses Logo component', async () => {
    const mobileHeaderPath = path.join(
      process.cwd(), 
      'src/components/layout/MobileHeader.tsx'
    );
