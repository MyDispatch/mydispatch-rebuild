...
test.describe('Design System Compliance (V18.3.27)', () => {
  ...
  test('No traffic light colors on icons', async ({ page }) => {
    ...
    const iconViolations = await page.evaluate(() => {
      ...
      const violations: string[] = [];
      const trafficColors = ['red', 'yellow', 'green', 'rgb(255, 0, 0)', 'rgb(255, 255, 0)', 'rgb(0, 255, 0)'];
      ...
      icons.forEach((icon) => {
        ...
        if (trafficColors.some(tc => color.includes(tc) || fill.includes(tc))) {
          violations.push(`Icon with class "${icon.className}" uses traffic light color: ${color || fill}`);
        }
      });
  ...
  test('No direct color usage (text-white, bg-black, etc.)', async ({ page }) => {
    ...
    const directColorUsage = await page.evaluate(() => {
      const allElements = document.querySelectorAll('*');
      const violations: string[] = [];
      const bannedClasses = ['text-white', 'text-black', 'bg-white', 'bg-black', 'border-white', 'border-black'];
      ...
      allElements.forEach((el) => {
        ...
        [...margins, ...paddings].forEach(prop => {
          ...
          if (value > 0 && value % 4 !== 0) {
            violations.push(`${el.tagName}.${el.className}: ${prop} = ${value}px (not on 4px grid)`);
          }
        });
      });
