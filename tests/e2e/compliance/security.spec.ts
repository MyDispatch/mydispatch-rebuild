...
test.describe('Security Compliance (V18.3.27)', () => {
  ...
  test('XSS protection: HTML entities are escaped', async ({ page }) => {
    ...
    if (await nameField.isVisible()) {
      ...
      const dialogVisible = await page.locator('text=XSS').isVisible().catch(() => false);
  ...
  test('Proper error messages (no stack traces)', async ({ page }) => {
    ...
    const errorMessage = await page.evaluate(() => {
      const errors = Array.from(document.querySelectorAll('[role="alert"], .error, .text-destructive'));
  ...
  test('Logout clears all session data', async ({ page }) => {
    ...
    const logoutButton = page.locator('button, a').filter({ hasText: /logout|abmelden/i }).first();
  ...
  test('GDPR: Data export functionality exists', async ({ page }) => {
    ...
    const hasExport = await page.evaluate(() => {
      ...
      return bodyText.includes('export') || 
  ...
  test('Soft delete implemented (archived flag)', async ({ page }) => {
    ...
    const deleteButtons = await page.locator('button').filter({ hasText: /delete|löschen|archiv/i }).count();
