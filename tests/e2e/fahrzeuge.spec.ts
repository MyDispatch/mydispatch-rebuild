...
test.describe('Fahrzeuge - Vehicle Management', () => {
  ...
  test('should display vehicle status filters', async ({ page }) => {
    ...
    const statusFilter = page.locator('[data-testid="status-filter"], text=/Status|Verfügbar|Im Einsatz/i').first();
  ...
  test('should display vehicle action buttons', async ({ page }) => {
    ...
    const addButton = page.locator('text=/Fahrzeug hinzufügen|Neues Fahrzeug|Add Vehicle/i').first();
