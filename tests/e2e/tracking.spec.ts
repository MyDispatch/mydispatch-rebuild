...
test.describe("Tracking - GPS Tracking Dashboard", () => {
  ...
  test("should display tracking map", async ({ page }) => {
    ...
    const mapContainer = page
      .locator('[data-testid="tracking-map"], [data-testid="here-map"], canvas')
      .first();
  ...
  test("should show active drivers list", async ({ page }) => {
    ...
    const driversList = page
      .locator('[data-testid="active-drivers"], text=/Aktive Fahrer/i')
      .first();
  ...
  test("should show real-time updates indicator", async ({ page }) => {
    ...
    const liveIndicator = page
      .locator('text=/Live|Echtzeit/i, [data-testid="live-indicator"]')
      .first();
  ...
  test("should display vehicle status filters", async ({ page }) => {
    ...
    const statusFilter = page
      .locator('[data-testid="status-filter"], text=/Status|Filter/i')
      .first();
  ...
  test("should not have console errors", async ({ page }) => {
    ...
    const criticalErrors = errors.filter(
      (err) =>
        !err.includes("tile load error") && !err.includes("Tangram") && !err.includes("favicon")
    );
