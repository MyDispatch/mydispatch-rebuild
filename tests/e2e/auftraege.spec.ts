...
test.describe("Order Creation Flow", () => {
  ...
  test("Vollständiger Auftrag: Kunde → Abholung → Fahrer → Abschluss", async ({ page }) => {
    ...
    const customerSelect = dialog
      .locator('[data-testid="customer-select"]')
      .or(dialog.locator('button:has-text("Kunde auswählen")'));
    ...
    await expect(page.locator("text=/erfolgreich erstellt|Auftrag angelegt/")).toBeVisible({
      timeout: 10000,
    });
    ...
    await expect(page.locator("text=/München Hauptbahnhof|München Flughafen/")).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test("Inline-Customer-Creation (Neuer Kunde während Auftragserstellung)", async ({ page }) => {
    ...
    // "Neuer Kunde" Button
    const newCustomerBtn = dialog
      .locator('button:has-text("Neuer Kunde")')
      .or(dialog.locator('[data-testid="create-customer-inline"]'));
  ...
  test("Validation: Pflichtfelder & Fehlerbehandlung", async ({ page }) => {
    ...
    await expect(dialog.locator("text=/erforderlich|Pflichtfeld|muss ausgefüllt/i")).toBeVisible({
      timeout: 3000,
    });
  ...
  test("Auftrag mit Airport-Pickup (Flugnummer)", async ({ page }) => {
    ...
    const airportCheckbox = dialog
      .locator('input[type="checkbox"]')
      .filter({ hasText: /Flughafen|Airport/ });
...
test.describe("ePOD - Elektronischer Liefernachweis", () => {
  ...
  test("Auftrag abschließen mit Signatur", async ({ page }) => {
    ...
    const confirmedBooking = page
      ...
      .filter({ hasText: /Bestätigt|Confirmed/ })
      .first();
    ...
    if (await confirmedBooking.isVisible({ timeout: 5000 })) {
      ...
      const completeBtn = dialog
        .locator('button:has-text("Abschließen")')
        .or(dialog.locator('button:has-text("Fertigstellen")'));

      if (await completeBtn.isVisible()) {
        await completeBtn.click();

        // ePOD-Dialog mit Signatur-Canvas
        const epodDialog = page
          .locator('[data-testid="epod-dialog"]')
          .or(page.locator("text=/Signatur/"));
        ...
        // Erwarte Success
        await expect(page.locator("text=/erfolgreich abgeschlossen|Abgeschlossen/")).toBeVisible({
          timeout: 10000,
        });
  ...
  test("ePOD-PDF herunterladen", async ({ page }) => {
    ...
    const completedBooking = page
      .locator('[data-testid="booking-row"]')
      .filter({ hasText: /Abgeschlossen|Completed/ })
      .first();
    ...
    if (await completedBooking.isVisible({ timeout: 5000 })) {
      ...
      const downloadBtn = dialog
        .locator('button:has-text("ePOD PDF")')
        .or(dialog.locator('[data-testid="download-epod"]'));
...
test.describe("Auftrags-Status-Workflow", () => {
  test("Status-Änderung: Pending → Confirmed → Completed", async ({ page }) => {
    ...
    const pendingBooking = page
      .locator('[data-testid="booking-row"]')
      .filter({ hasText: /Ausstehend|Pending/ })
      .first();
    ...
    if (await pendingBooking.isVisible({ timeout: 5000 })) {
      ...
      const statusSelect = dialog
        ...
        .or(dialog.locator('select[name="status"]'));
      ...
      if (await statusSelect.isVisible()) {
        ...
        // Erwarte Success
        await expect(page.locator("text=/aktualisiert|gespeichert/")).toBeVisible({
          timeout: 5000,
        });
  ...
  test("Auftrag stornieren mit Grund", async ({ page }) => {
    ...
    if (await cancelBtn.isVisible()) {
      ...
      const confirmDialog = page
        ...
        .or(page.locator("text=/Möchten Sie wirklich/"));
  ...
  test("Mehrere Aufträge auswählen & Status ändern", async ({ page }) => {
    ...
    if (count >= 2) {
      ...
      if (await statusChangeBtn.isVisible()) {
        ...
        await expect(page.locator("text=/Aufträge aktualisiert|erfolgreich geändert/")).toBeVisible(
          { timeout: 10000 }
        );
      }
    }
...
test.describe("Suche & Filter", () => {
  test("Suche nach Kundennamen", async ({ page }) => {
    ...
    const searchInput = page
      .locator('input[placeholder*="Suche"]')
  ...
  test("Filter nach Status", async ({ page }) => {
    ...
    const statusFilter = page
      ...
      .or(page.locator('select:has-text("Status")'));
