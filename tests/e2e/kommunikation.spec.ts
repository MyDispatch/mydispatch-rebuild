...
test.describe("Kommunikation - Communication Dashboard", () => {
  ...
  test("should display communication dashboard", async ({ page }) => {
    await expect(page.locator("text=/Kommunikation|Communication/i").first()).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test("should show new message button", async ({ page }) => {
    const newMessageButton = page
      .locator("text=/Neue Nachricht|New Message|Nachricht senden/i")
      .first();
  ...
  test("should filter messages by type", async ({ page }) => {
    const filterOptions = page
      .locator('[data-testid="message-filter"], text=/Alle|Ungelesen|Archiviert/i')
      .first();
