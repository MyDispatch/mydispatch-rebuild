    ...
    switch (action) {
      case 'validate': {
        ...
        if (request.confidence >= CONFIDENCE_THRESHOLD) {
          ...
          console.info(`[Auto-Approve] Confidence ${request.confidence} >= ${CONFIDENCE_THRESHOLD}`);
          ...
          console.warn(`[NeXify Review] Confidence ${request.confidence} < ${CONFIDENCE_THRESHOLD}`);
