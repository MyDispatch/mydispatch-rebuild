...
export const PRICING_TEXTS = {
  tariffs: ${JSON.stringify(tariffs.map(t => ({ id: t.tariff_id, title: t.marketing_title, subtitle: t.marketing_subtitle })), null, 2)},
  addOns: ${JSON.stringify(addOns.map(a => ({ id: a.add_on_id, name: a.name, description: a.description })), null, 2)}
};
  `.trim();
}
