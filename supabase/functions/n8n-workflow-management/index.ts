...
serve(async (req) => {
  ...
  try {
    const {
      ...
      workflow_data,
      limit = 50,
    } = (await req.json()) as WorkflowRequest;
    ...
    console.log(
      `[n8n Management] Action: ${action}, Base URL: ${baseApiUrl}, Instance: ${n8nInstanceUrl}`
    );
    ...
    switch (action) {
      ...
      case "update_workflow":
        if (!workflow_id || !workflow_data)
          throw new Error("workflow_id and workflow_data required");
        endpoint = `/workflows/${workflow_id}`;
        method = "PUT";
        body = workflow_data;
        break;
