...
const bookingSchema = z.object({
  pickup_address: z
    .string()
    .trim()
    .min(5, "Abholadresse muss mindestens 5 Zeichen lang sein")
    .max(500, "Abholadresse zu lang"),
  dropoff_address: z
    .string()
    .trim()
    .min(5, "Zieladresse muss mindestens 5 Zeichen lang sein")
    .max(500, "Zieladresse zu lang"),
  pickup_time: z
    .string()
    .datetime({ message: "Ungültiges Datum-Format" })
    .refine((date) => new Date(date) > new Date(), "Abholzeit muss in der Zukunft liegen"),
  vehicle_type: z.enum([
  ...
  luggage: z
    .number()
    .int()
    .min(0, "Gepäck darf nicht negativ sein")
...
serve(async (req) => {
  ...
  try {
    ...
    if (customer.company_id !== validated.company_id) {
      console.error(
        `[Portal Booking] Company mismatch: ${customer.company_id} vs ${validated.company_id}`
      );
      return new Response(JSON.stringify({ error: "Unternehmens-ID stimmt nicht überein" }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
