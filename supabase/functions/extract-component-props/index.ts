...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `[EXTRACT-PROPS] Extracting props${component_path ? ` for: ${component_path}` : " for all components"}`
    );
    ...
    for (const component of mockComponents) {
      const { error } = await supabase.from("component_registry").upsert(component, {
