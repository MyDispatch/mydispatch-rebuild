...
serve(async (req) => {
  ...
  try {
    ...
    try {
      const criticalTables = ['companies', 'profiles', 'bookings', 'customers', 'drivers', 'vehicles'];
      let tablesExist = 0;
    ...
    console.log(`[Phase 3] Validation complete. Score: ${overallScore.toFixed(1)}%, Confidence: ${(averageConfidence * 100).toFixed(1)}%`);
