...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const {
      ...
      task_type,
      affected_files = [],
    }: KnowledgeCheckRequest = await req.json();
    ...
    const checklist: string[] = ["✅ Knowledge-Base-Check durchgeführt"];

    if (task_type === "component_creation" && componentsData.length > 0) {
      checklist.push(
        `⚠️ ${componentsData.length} ähnliche Component(s) gefunden - prüfe ob Wiederverwendung möglich!`
      );
    }

    if (issuesData && issuesData.length > 0) {
      checklist.push(
        `⚠️ ${issuesData.length} bekannte Issue(s) gefunden - beachte Prevention-Checklist!`
      );
    }
    ...
    if (
      ...
      keywords.some((k) => ["style", "color", "design"].includes(k))
    ) {
    ...
    const totalFindings =
      (knowledgeData?.length || 0) + (snippetsData?.length || 0) + (practicesData?.length || 0);
