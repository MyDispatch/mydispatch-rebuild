...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { period = "last_7_days", force = false }: SelfReportRequest = await req
      .json()
      .catch(() => ({}));
    ...
    const total_pattern_applications =
      patterns?.reduce((sum, p) => sum + (p.applied_count || 0), 0) || 0;
    ...
    const auto_fix_success_rate =
      auto_fix_actions.length > 0 ? auto_fix_successes / auto_fix_actions.length : 0;
    ...
    if (knowledge_check_compliance < 1.0) {
      identified_gaps.push(
        `Knowledge Check Compliance bei ${Math.round(knowledge_check_compliance * 100)}% (Ziel: 100%)`
      );
    }
    if (hallucination_rate > 0.01) {
      identified_gaps.push(
        `Hallucination Rate bei ${Math.round(hallucination_rate * 100)}% (Ziel: <1%)`
      );
    }
    if (pattern_reuse_rate < 0.8) {
      identified_gaps.push(
        `Pattern Reuse Rate bei ${Math.round(pattern_reuse_rate * 100)}% (Ziel: >80%)`
      );
    }
    if (documentation_freshness_days > 30) {
      identified_gaps.push(
        `Dokumentation ${documentation_freshness_days} Tage alt (Ziel: <30 Tage)`
      );
    }
