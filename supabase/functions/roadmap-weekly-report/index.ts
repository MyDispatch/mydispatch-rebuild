...
serve(async (req) => {
  ...
  try {
    ...
    // Berechne Velocity
    const totalHours = (completedTasks || []).reduce((sum, t) => sum + (t.actual_hours || t.estimated_hours || 0), 0);
    ...
    const completionPercent = totalTasks ? Math.round((completedCount || 0) / totalTasks * 100) : 0;
