    ...
    const criticalDocs: DocEntry[] = [
      {
        ...
        content: {
          ...
          v28_components: [
            { name: 'V28Button', path: 'src/components/v28/V28Button.tsx', variants: ['primary', 'secondary', 'outline', 'ghost'] },
            { name: 'V28Card', path: 'src/components/v28/V28Card.tsx', usage: 'Container with slate-palette styling' },
            { name: 'V28IconBox', path: 'src/components/v28/V28IconBox.tsx', usage: 'Icon container with consistent spacing' },
            { name: 'V28MarketingCard', path: 'src/components/v28/V28MarketingCard.tsx', usage: 'Marketing content cards' }
          ],
          ...
          issues: [
            {
              ...
              prevention: ['Check filesExplorer.md', 'Query component_registry', 'Never code from memory']
            },
            {
              ...
              prevention: ['Run security linter', 'Check RLS on every new table', 'Use supabase--linter tool']
            },
            {
              ...
              prevention: ['Review tailwind.config.ts', 'Never use direct colors', 'Always use design system']
            }
          ]
        },
      ...
      },
      {
        ...
        content: {
          critical_rule: 'NEVER use designTokens.colors.primary.DEFAULT - ONLY Tailwind-native slate-palette',
          color_palette: {
          ...
          components: [
            'V28Button', 'V28Card', 'V28IconBox', 'V28MarketingCard'
          ],
      ...
      },
      {
        ...
        content: {
          phases: [
            { phase: 1, name: 'Pattern Recognition', description: 'Analyze repeated errors and successful patterns' },
            { phase: 2, name: 'Solution Development', description: 'Create Edge Functions, Hooks, Utils autonomously' },
            { phase: 3, name: 'Self-Testing', description: 'Validate solutions with automated tests' },
            { phase: 4, name: 'Auto-Documentation', description: 'Document in knowledge_base, code_snippets, best_practices' },
            { phase: 5, name: 'Self-Utilization', description: 'Use own solutions in future implementations' }
          ],
      ...
      },
      {
        ...
        content: {
          ...
          example: 'CREATE POLICY "Users can view their own data" ON table FOR SELECT USING (auth.uid() = user_id);',
      ...
      },
      {
        ...
        content: {
          ...
          cors_template: 'const corsHeaders = { "Access-Control-Allow-Origin": "*", "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type" };'
        },
      ...
      },
      {
        ...
        category: 'best_practice',
        content: {
          pattern: 'try { ... } catch (error) { const errorMessage = error instanceof Error ? error.message : "Unknown error"; }',
      ...
      },
      {
        ...
        category: 'component_pattern',
        content: {
          pattern: 'const { data, isLoading, error } = useQuery({ queryKey: ["key"], queryFn: async () => {...} });',
          best_practices: [
