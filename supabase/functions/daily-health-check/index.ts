  ...
  try {
    ...
    try {
      ...
      if (responseTime > 1000) {
        result.checks.database = { status: 'warning', message: `Slow response: ${responseTime}ms`, responseTime };
    ...
    try {
      ...
      if (errorLogs && errorLogs.length > 0) {
        ...
        if (errorRate > 0.1) {
          ...
          result.issues.push(`Elevated error rate: ${(errorRate * 100).toFixed(1)}% (threshold: 5%)`);
    ...
    try {
      ...
      const { data: slowQueries, error } = await supabase
        .rpc('check_slow_queries', { threshold_ms: 500 });
      ...
      if (error) {
        ...
        result.checks.performance = { status: 'warning', message: `${slowQueries.length} slow queries detected` };
