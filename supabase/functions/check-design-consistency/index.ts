...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { page_path, compare_with = ["src/pages/Index.tsx"] }: DesignCheckRequest =
      await req.json();
    ...
    const harmony_score =
      inconsistencies.length === 0 ? 1.0 : Math.max(0.5, 1.0 - inconsistencies.length * 0.1);
    ...
    console.log("✅ Design Check Completed:", {
      harmony_score,
