...
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public', 'pg_catalog'  -- ✅ FIX: Immutable search_path
AS $$
BEGIN
...
COMMENT ON FUNCTION public.get_dashboard_stats_for_company IS 'Returns exactly 1 row with dashboard stats (0-values if no data). Fixed: replaced invalid ''unpaid'' with valid ENUM values + secure search_path.';
