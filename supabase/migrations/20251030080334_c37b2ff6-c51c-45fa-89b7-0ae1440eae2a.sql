-- =====================================================================
-- PHASE 1: KNOWLEDGE-BASE DATABASE SCHEMA V5.0
...
CREATE TABLE IF NOT EXISTS public.knowledge_validation_log (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Public READ access for AI to learn from knowledge
CREATE POLICY "AI can read all knowledge"
  ON public.knowledge_base FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "AI can read all code snippets"
  ON public.code_snippets FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "AI can read all best practices"
  ON public.best_practices FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "AI can read component registry"
  ON public.component_registry FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "AI can read known issues"
  ON public.known_issues FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "AI can read learning patterns"
  ON public.ai_learning_patterns FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "AI can read actions log"
  ON public.ai_actions_log FOR SELECT
  TO authenticated
  USING (true);

-- WRITE access for AI to learn and document
CREATE POLICY "AI can insert knowledge"
  ON public.knowledge_base FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "AI can update knowledge"
  ON public.knowledge_base FOR UPDATE
  TO authenticated
  USING (true);

CREATE POLICY "AI can insert code snippets"
  ON public.code_snippets FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "AI can update code snippets"
  ON public.code_snippets FOR UPDATE
  TO authenticated
  USING (true);

CREATE POLICY "AI can insert best practices"
  ON public.best_practices FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "AI can insert components"
  ON public.component_registry FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "AI can update components"
  ON public.component_registry FOR UPDATE
  TO authenticated
  USING (true);

CREATE POLICY "AI can insert known issues"
  ON public.known_issues FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "AI can update known issues"
  ON public.known_issues FOR UPDATE
  TO authenticated
  USING (true);

CREATE POLICY "AI can insert learning patterns"
  ON public.ai_learning_patterns FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "AI can insert actions log"
  ON public.ai_actions_log FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "AI can insert validation log"
  ON public.knowledge_validation_log FOR INSERT
  TO authenticated
  WITH CHECK (true);

-- Admin full access
CREATE POLICY "Admins have full access to knowledge"
...
-- Function to increment access counts
...
-- Function to update issue occurrences
...
COMMENT ON TABLE public.knowledge_base IS 'Central knowledge repository for AI agent - stores all learned patterns, best practices, and documentation';
COMMENT ON TABLE public.code_snippets IS 'Reusable code patterns with usage tracking and success rates';
COMMENT ON TABLE public.best_practices IS 'Do and Don''t guidelines for development';
COMMENT ON TABLE public.component_registry IS 'Registry of all existing components to prevent recreation';
COMMENT ON TABLE public.known_issues IS 'Known bugs and anti-patterns with solutions';
COMMENT ON TABLE public.ai_learning_patterns IS 'AI self-learning patterns from successful/failed actions';
COMMENT ON TABLE public.ai_actions_log IS 'Complete log of all AI actions for audit and improvement';
COMMENT ON TABLE public.knowledge_validation_log IS 'Quality tracking for knowledge base entries';