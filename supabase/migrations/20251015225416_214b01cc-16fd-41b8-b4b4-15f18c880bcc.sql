-- ==================================================================================
-- DATENBANK-MIGRATION V18.1: VOLLSTÄNDIGE FORMULARFELDER
-- ==================================================================================
...
-- Constraint für customer_type
ALTER TABLE customers ADD CONSTRAINT customers_customer_type_check 
  CHECK (customer_type IN ('Privatkunde', 'Geschäftskunde'));

-- Constraint für tax_id (nur bei Geschäftskunden)
ALTER TABLE customers ADD CONSTRAINT customers_tax_id_check 
  CHECK (
    (customer_type = 'Geschäftskunde' AND tax_id IS NOT NULL) 
    OR (customer_type = 'Privatkunde')
  );

-- Constraint für company_name (nur bei Geschäftskunden)
ALTER TABLE customers ADD CONSTRAINT customers_company_name_check 
  CHECK (
    (customer_type = 'Geschäftskunde' AND company_name IS NOT NULL) 
    OR (customer_type = 'Privatkunde')
  );
